...
- **Offline by default, one hardened network path.** Every query works with zero
  ...
  HTTPS-only, credential- and downgrade-rejecting, size-capped, conditional when
  it can be, published append-only. This is stricter than network-first designs
  and pairs with the reproducibility story rather than fighting it.
