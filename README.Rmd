...
output:
  github_document:
    html_preview: false
    # Pin the wrap mode so README.md renders byte-identically on any machine
    # (pandoc's default --wrap=auto re-flows prose at a version-dependent
    # width). `preserve` mirrors the line wrapping of this source file, so a
    # prose edit here produces a matching, reviewable diff in README.md.
...
# Render against an empty snapshot cache so this block is byte-identical on
# every machine (the CI gate re-renders README.md and diffs it). Without this,
# the output would reflect whatever the rendering machine happens to have
# refreshed.
options(
  pslr.cache_dir = file.path(tempdir(), "pslr-readme-cache"),
  pslr.config_dir = file.path(tempdir(), "pslr-readme-config")
)
```
