...
**Generated by `data-raw/snapshot-tracker.sh` on 2026-09-09.**
225 issues. Do not hand-edit: the next run overwrites this file
...
PSLR-cmkufiww [in-progress] [critical] Cut pslr 1.2.0: index back to -v1, drop the Remotes pin, refresh cran-comments

PSLR-hrpalwzo [done] [critical] Orchestrate CRAN release chain: pslr -> punycoder -> rurl
├── PSLR-kwareoss [todo] [critical] Phase R: gate, submit and verify rurl 3.0.0
│   ├── PSLR-wzrevacl [in-progress] [critical] R5: verify rurl 3.0.0 is accepted and live on CRAN
│   ├── PSLR-uwcwkfwm [todo] [critical] R3: run the cran gate for rurl 3.0.0 in repair and remote mode
│   ├── PSLR-tcgxnrek [done] [critical] R2: remove rurl Remotes and prove CRAN-only resolution
│   ├── PSLR-lararika [done] [critical] R4: submit the approved rurl 3.0.0 release to CRAN
...
├── PSLR-wqnvvrch [done] [low] Decide \() shorthand vs function() policy (R 4.0.0 vs 4.1.0 floor) across pslr/punycoder/rurl
└── PSLR-gzgwztas [done] [low] Option C: route default/rfc3986 file:// out of libcurl for deterministic cross-platform parsing

PSLR-thcaqtnw [done] [critical] GitHub account suspended: DESCRIPTION URL and BugReports 404 on CRAN today, blocking the release chain
├── PSLR-totktvlq [done] [critical] No CI: migrate the nine GitHub Actions workflows to GitLab CI
│   ├── PSLR-ugxanxne [done] [high] Replace the GitLab schedules with a local verify gate (runner minutes exhausted)
│   │   └── PSLR-lidgnomk [done] [high] verify.sh reads OSSINDEX_TOKEN from the shell but tells you to put it in ~/.Renviron: the OSS Index audit never runs
│   └── PSLR-avwlybsw [done] [high] Restore ASAN/UBSAN/valgrind coverage of src/ as a verify.sh sanitize tier
├── PSLR-cftgnuxo [done] [high] Rewrite .bestpractices.json: 45 dead GitHub URLs and five clusters of false claims
│   ├── PSLR-xogfazqo [todo] [medium] Update the live OpenSSF badge entry on bestpractices.dev to match the corrected local file
│   └── PSLR-boiuqhnr [done] [medium] Two committed comments still describe the v* tag pipeline trigger that 8a0dc2d removed
└── PSLR-wclmdkju [done] [high] Publish the pkgdown site from a local runner and keep agent-instruction files out of it

PSLR-zibafvbq [done] [critical] cran tier reports a pass on an R CMD check that aborted

PSLR-rnfnzwqu [done] [critical] Make the profile-rebuild and dedup tests agnostic to the installed punycoder profile
...
PSLR-epkkemop [done] [high] site/ is missing from .Rbuildignore: pkgdown output ships in the tarball

PSLR-qjsszyig [done] [high] A bundled-snapshot bump leaves README.md advertising the previous snapshot; nothing in the bump procedure regenerates it
└── PSLR-wydqlubg [todo] [low] Should README.md print the snapshot checksum and timestamps at all? (panel split on PSLR-qjsszyig)
...
PSLR-girwpagy [done] [medium] psl_snapshots() reports a stale normalization_profile after a punycoder change

PSLR-ucvugmsw [done] [medium] Bundled PSL snapshot is behind upstream: pinned 9186eee, latest e1b8015

PSLR-wcjcywea [done] [medium] CI installs pandoc from a hardcoded amd64 .deb: every job dies on an arm64 runner

PSLR-aquayvhw [done] [low] Add @examples to pslr-package.Rd (closes help_examples_coverage gap)

PSLR-fqdepqem [done] [low] Retire the Claude auto-memory store: promote what is durable, delete the rest
...
225 stanzas, in sorted short-id order. Each is `fp context` output
...
## PSLR-boiuqhnr: Two committed comments still describe the v* tag pipeline trigger that 8a0dc2d removed

**Status:** done | **Parent:** cftgnuxohcnqkjplqcuiqcnnctsjqbix

### Description

Two committed comments still describe a `v*` tag trigger that `8a0dc2d` removed.
Same defect class as the `.bestpractices.json` cluster-3 justifications fixed
under PSLR-cftgnuxo, found the same way — by reading each claim against
`.gitlab-ci.yml`'s actual `workflow:` rules rather than against the previous
description of them.

Ground truth, from the `workflow:` block: a pipeline assembles only for
`$CRAN_PREP == "1"` or `$DEPLOY_PAGES == "1"` raised from a `web`, `api` or
`trigger` source, and `when: never` otherwise. No branch push, no merge request,
**no tag**, no schedule creates one.

### 1. `tools/verify.sh`, header comment
...
"for releases" is wrong: nothing about a release, a tag, or a version creates a
pipeline. Only a named manual trigger does. This matters more than an ordinary
stale comment because the sentence tells the reader to go check the `workflow:`
rules, lending itself authority it no longer has.

### 2. `.gitlab-ci.yml`, the `pages:` job comment

    # When this actually publishes is decided by the `workflow:` rules at the top of
    # the file, not by the rules below: branch pushes create no pipeline at all, so
    # in practice the site rebuilds on a `v*` release tag or on demand via
    # `glab ci run --branch main`.

Self-contradictory as well as stale: the same sentence correctly says the
`workflow:` rules decide, correctly says branch pushes create nothing, and then
asserts a tag rebuild those rules do not permit. The `on demand` half is right
and is the whole story — `glab ci run --branch main --variables DEPLOY_PAGES:1`.

The following sentence ("Between releases the published site therefore documents
the last release, which is the right default for a CRAN package") rests on the
tag trigger and is false as a consequence: with no automatic publish at all, the
site documents whichever commit someone last ran `DEPLOY_PAGES=1` against. That
may be a weaker default than the comment claims and is worth stating plainly
rather than papering over.

### Scope

Correct both to the two-named-pipeline model. `docs/git-workflow.md`'s "The
remote pipeline" section was checked in the same pass and is **accurate** — it
describes `CRAN_PREP=1` and `DEPLOY_PAGES=1` correctly and needs no change; use
it as the reference wording.

### Done when

No comment in the tree describes a tag-triggered, push-triggered or scheduled
pipeline; `grep -rn 'v\*' tools/ .gitlab-ci.yml` turns up no surviving claim of
a tag trigger.

### Provenance

Surfaced by the PSLR-cftgnuxo subagent while establishing ground truth for the
badge justifications, and re-verified against the tree before filing.
...
Landed in MR !23, commit 2e736e2.

`tools/verify.sh`'s header no longer claims the hosted pipeline runs "for
releases and manual triggers" — it names the two pipelines and states plainly
that nothing fires by itself: not a push, not a merge request, not a tag, not a
schedule.

`.gitlab-ci.yml`'s `pages:` comment is corrected the same way, and so is the
sentence after it, which this issue flagged as false-by-consequence. "Between
releases the published site documents the last release" is only true if a
release publishes it; with no automatic publish the site documents whichever
commit someone last ran `DEPLOY_PAGES=1` against. The comment now says that, and
says it is a weaker default than the one it used to claim, rather than papering
over the difference.

Done-when condition met: `grep -rn 'v\*' tools/ .gitlab-ci.yml` returns only the
two `git tag -l 'v*'` uses in the news-version check, which are tag *reads*, not
pipeline triggers.

Comments only, no behaviour change. `docs/git-workflow.md`'s "The remote
pipeline" section was re-checked and needed nothing, and supplied the reference
wording as this issue proposed.

Gate: `tools/verify.sh full`, exit 0, every step green.
...
**Status:** done | **Parent:** thcaqtnwxwdkbcfdvtqprgdxbefszcnl
...
Unblocked, and most of the work turns out to be already in the tree. Read from the file today, not inferred.

The BLOCKED note in the previous comment rested on `visibility: private`. That is no longer true: the project is `visibility: public` with `pages_access_level: public`, so the six MUST criteria that a private repository fails (repo_public, repo_track, repo_interim, report_url, report_archive, discussion) are satisfiable again, and the badge stands. The README badge at line 22 stays.

`.bestpractices.json` was already rewritten as part of 1c25189. Measured:

    0   occurrences of "github" in the file
    46  occurrences of "gitlab"
    60  criteria Met, 15 N/A, 0 Unmet

The 45 dead justification URLs are gone. Spot-checked the three clusters this issue called out as substantive rather than cosmetic:

  cluster 2 (vulnerability reporting) — now describes email to the maintainer as the sole confidential channel and cites SECURITY.md, explicitly noting the channel does not depend on any forge account's status. That is the honest claim and it matches the SECURITY.md rewrite in 0715464.
  cluster 4 (sanitizers) — now cites `tools/verify.sh sanitize` under Docker as part of the `cran` tier, which is a real mechanism (PSLR-avwlybsw), not the dead rhub.yaml.
  cluster 5 — pkgdown, repository and coverage links all point at GitLab.

Cluster 3 (CI cadence) needs one more pass and it is now MORE wrong, not less: the previous comment recorded the hosted pipeline as running "only on v* tags and manual trigger", and as of 8a0dc2d that is no longer accurate either — there are two named pipelines (`CRAN_PREP=1`, `DEPLOY_PAGES=1`) and no tag trigger at all. Any justification mentioning tag-triggered or per-push CI needs re-reading against the current file.

Scope correction from the previous comment stands and was re-verified: `.fossa.yml`'s `id: git+github.com/bart-turczynski/pslr` is FOSSA's project key, not a URL. Both README badges encode the same string and both resolve 200 today; repointing orphans the scan history. Left as is, and the file carries a comment saying why.

REMAINING, and both are yours rather than mine:
  1. Re-read the cluster-3 justifications against the two-pipeline model and correct any that still describe tag or per-push CI.
  2. The live badge entry on bestpractices.dev holds the text a reviewer actually reads. This file is only the local copy; the live entry must be updated by hand. Out of scope as the issue says, but it is the half that counts.

#### 2026-09-05 — bartek@turczynski.pl

The in-repo half is done. MR !20, commits 16e3163 + 8fa2bdb, merged to main.

Remaining item 1 from the 2026-09-05 comment — re-read the cluster-3
justifications against the two-pipeline model — is complete. Six were corrected
(`contribution_requirements`, `test_most`, `warnings`, `static_analysis_often`,
`dynamic_analysis_enable_assertions`, `test_continuous_integration`), each
falsified first against a quoted line of the live config. The `coverage` and
`lint` jobs extend `.rules-cran-prep`, so they run under `CRAN_PREP=1` and not
"every hosted pipeline"; `standard` deliberately excludes `R CMD check`; the
matrix is devel / 4.6 / 4.5, not devel/release/oldrel-1.

**One status flipped to Unmet**: `test_continuous_integration`. Stated rather
than described around, per this issue's own rule. No hosted pipeline runs on an
integration event, and the per-change gate is a local pre-push hook a reviewer
can neither inspect nor verify and `--no-verify` bypasses. Confirmed against the
2026-08-09 finding that this criterion is SUGGESTED, not MUST, so the badge
stands and the README badge at line 22 stays. `static_analysis_often` was kept
Met — its criterion is about frequency, which the pre-push hook genuinely
satisfies — but its justification now states the unverifiability and the bypass
explicitly.

**Three further false claims found by reading the whole file rather than only
the named suspects**, kept in a separate commit:

  - `warnings_strict` cited `LINTR_ERROR_ON_LINT=true`. That variable exists
    nowhere in the repository except that sentence — an invented mechanism, not
    a stale one. The real one is `lintr::lint_package()` exiting non-zero.
  - `static_analysis` claimed goodpractice is run. It is not, anywhere. What is
    true is that `.lintr` keeps lintr's linter set in step with goodpractice's.
  - `dynamic_analysis` claimed `tools/verify.sh sanitize` runs `R CMD check`.
    `run_sanitize()` runs `R CMD INSTALL` then `testthat::test_local()`.

File structure preserved: 150 keys, byte-identical key order, valid JSON, 0
occurrences of `github`. Statuses now Met 59 / N/A 15 / Unmet 1.

The scope corrections from earlier comments were re-verified and stand:
`.fossa.yml`'s `id: git+github.com/...` is FOSSA's project key, both README
badges encode it, both resolve 200, and repointing it registers a second empty
project rather than moving the existing one. Left alone.

Two things came out of this that are now their own issues rather than buried
here:

  - **PSLR-boiuqhnr** (child of this): `tools/verify.sh`'s header comment and
    `.gitlab-ci.yml`'s `pages:` comment still describe a `v*` tag trigger that
    8a0dc2d removed. Same defect class, found the same way.
  - **PSLR-xogfazqo**: the live bestpractices.dev entry. This file is only the
    local copy and the live text is what a reviewer reads; it needs the same
    six corrections plus the Unmet flip, by hand, on the web. That was always
    out of scope here, but it is the half that counts and should not vanish
    with this issue.

Also noted and not acted on: `no_leaked_credentials` and both `static_analysis*`
entries claim FOSSA scans run. No FOSSA invocation exists in `.gitlab-ci.yml` or
`tools/verify.sh` — only `.fossa.yml` and the README badges — so the scan is
presumably driven from the FOSSA side. Unverified either way; worth confirming
...
## PSLR-cmkufiww: Cut pslr 1.2.0: index back to -v1, drop the Remotes pin, refresh cran-comments

**Status:** in-progress

### Description

Cut pslr 1.2.0 — step 1 of the three-step order recorded on PSLR-fjkaqckg, and
the release that unblocks punycoder 1.3.0.

Version set to 1.2.0 by the maintainer (2026-09-09). Minor rather than patch:
the release carries a breaking rename in `psl_snapshots()` (PSLR-girwpagy), a
new `psl_diff()` (PSLR-aeuaykkf) and a bundled-snapshot refresh that changes
query results for real domains (PSLR-ucvugmsw).

### Why this release exists

PSLR-rnfnzwqu made pslr's suite pass under both punycoder normalization
profiles. Publishing that is what lets punycoder submit 1.3.0 without CRAN's
reverse-dependency check failing against pslr.

### Work

1. **Regenerate `R/sysdata.rda` under CRAN punycoder 1.2.1**, so the shipped
   index carries the `-v1` / 16.0.0 stamp. It currently carries `-v2` / 17.0.0
   from the dev-pin work in PR #109.

   This is the decision measured on PSLR-fjkaqckg: the profile-mismatch rebuild
   costs 2.75 s per session, and shipping `-v2` while CRAN's punycoder is 1.2.1
   charges that to 100% of users immediately. Shipping `-v1` charges nobody
   until punycoder 1.3.0 lands.

   Run `Rscript data-raw/update_psl.R` with punycoder 1.2.1 on the library
   path. The pinned upstream commit does not move — it stays `46ae48ce`, so
   only the normalizer identity in `meta` should change. Anything else in the
   diff is a finding, not a regeneration.

2. **Drop the `Remotes:` dev pin** (`DESCRIPTION:44`,
   `gitlab::bart-turczynski/punycoder@ec2643c7…`). It exists so CI resolves the
   normalizer the `-v2` index was built under; with the index back on `-v1`
   there is nothing to pin, and CRAN ignores the field anyway.

3. **Leave the `Imports` floor at `punycoder (>= 1.1.0)`.** Raising it to
   `>= 1.3.0` is PSLR-uxklngwq step 4 and belongs to the THIRD release, not
   this one — 1.3.0 does not exist yet.

4. **Version and NEWS.** `DESCRIPTION` `Version: 1.2.0`; the top `NEWS.md`
   heading becomes `# pslr 1.2.0`. The `news-version` check requires they
   match.

5. **Rewrite `cran-comments.md`.** It currently documents the already-shipped
   1.0.1 -> 1.1.1 release.

### Verification

Full `R CMD check --as-cran` on the built tarball against BOTH punycoder
populations — CRAN 1.2.1 and dev 1.2.1.9000. Running against one is the defect
PSLR-rnfnzwqu existed to fix; do not regress into it.

### Known gate noise

The pre-push verify gate is red on `test-security.R:23` for reasons unrelated
to this repository — see SEOR-fkvlzltx in the seor backlog. It skips under
`R CMD check`, so the `--as-cran` verification above is unaffected.

### Not in scope

Reshipping the index under 17.0.0 and raising the floor (PSLR-fjkaqckg,
PSLR-uxklngwq step 4). Those are the third release, after punycoder 1.3.0 is
live on CRAN.


### Comments

#### 2026-09-09 — bartek@turczynski.pl

RELEASE PREPARED AND MERGED, NOT SUBMITTED. MR !27, merge commit ba16881.

Steps 1, 2, 4 and 5 are done: index regenerated under CRAN punycoder 1.2.1
(`-v1` / 16.0.0), `Remotes:` dropped, `Version: 1.2.0` with a matching NEWS
heading, `cran-comments.md` rewritten. Step 3 held as specified -- the `Imports`
floor stays at `punycoder (>= 1.1.0)`.

Regeneration moved only what it should: same pinned commit `46ae48ce`, same
checksum, same 10,323 rules, `identical()` TRUE across all seven columns; only
`retrieved_at`, `normalizer_version`, `normalization_profile` and
`unicode_version` differ.

Verification, both punycoder populations plus the reverse dependency:

    R CMD check --as-cran vs punycoder 1.2.1        Status: 1 NOTE
    R CMD check --as-cran vs punycoder 1.2.1.9000   Status: 2 NOTES
    R CMD check rurl_3.0.1 vs pslr 1.2.0 + 1.2.1    Status: OK

The single NOTE in the submission condition is the `/-/issues` URL returning
404, which GitLab does site-wide for unauthenticated clients
(`gitlab-org/gitlab` behaves identically). Every other declared URL returns
200. cran-comments.md pre-empts it.

NEW REVERSE DEPENDENCY, and nothing in this backlog would have surfaced it:
rurl reached CRAN as **3.0.1** on 2026-09-09 at 07:40 UTC. It is a live revdep
of pslr for the first time. Its check against this release is clean. Recorded
on seor SEOR-kbsrmutw.

NOT DONE, and deliberately: the CRAN submission itself. That is an
owner-authorized action and has not been taken. What remains is a maintainer
running the submission and, on acceptance, tagging `v1.2.0`.

The one thing worth reading before submitting is the examples-timing finding
now recorded on PSLR-fjkaqckg -- it does not block this release, but it puts a
deadline on the one after it.
...
## PSLR-epkkemop: site/ is missing from .Rbuildignore: pkgdown output ships in the tarball

**Status:** done

### Description

Found by a full `tools/verify.sh cran` run on 2026-09-05. All three Docker matrix legs (R 4.5, 4.6, devel) raise:

    * checking top-level files ... NOTE
    Non-standard file/directory found at top level:
      'site'

and R CMD build reports `Removed empty directory 'pslr/site/tutorials'`, confirming the directory is being carried into the tarball.

### Cause

_pkgdown.yml sets `destination: site` (docs/ is committed source, not a pkgdown build). site/ is gitignored and holds 3.5 MB of built documentation. .Rbuildignore has `^docs$` and `^_pkgdown\.yml$` but never gained `^site$` when the destination changed.

### Fix

Add `^site$` to .Rbuildignore.

### Acceptance criteria

- R CMD check no longer reports a non-standard top-level 'site' directory.
- The built tarball does not contain site/.
- NEWS.md bullet, since package contents change.
...
Fixed in MR !16. .Rbuildignore gains ^site$; a real remote as-cran run no longer reports a non-standard top-level 'site' directory (3 NOTEs down to 1).
...
ORDERING RULE UPHELD, and now with a number behind it (2026-09-09).

This issue's rule -- do not reship the index under 17.0.0 until punycoder 1.3.0
is live -- was challenged in a second-opinion review, which argued the opposite:
ship the `-v2` index in the next pslr release and skip a third submission
entirely, since users would land on the fast path as soon as they upgraded
punycoder. The argument turns on the rebuild being cheap. It is not.

### MEASURED: the rebuild costs 2.75 s per session

punycoder 1.2.1 (CRAN, `-v1`) in a throwaway library, against pslr's dev tree
whose `sysdata.rda` carries the `-v2` stamp, so the mismatch path is the one
taken. `rebuild_bundled_rules()`, 5 runs, elapsed seconds:

    2.791  2.753  2.815  2.734  2.717      min 2.717, median 2.753

10,323 rules re-parsed and re-normalized, once per session, on first use, and
it blocks. The cost is symmetric -- a `-v1` index under a `-v2` runtime is the
same re-parse.

That settles who should pay. Shipping `-v2` now charges 2.75 s per session to
100% of pslr users, starting immediately, because punycoder 1.3.0 does not
exist yet and users upgrade lazily. Shipping `-v1` charges nobody until 1.3.0
lands, and then only users who upgrade punycoder, until pslr N+2. The third
submission is the correct price. The reviewer agreed once the measurement was
supplied.

One correction to absorb from that review: N+2's *submission* timing is
controllable, its CRAN acceptance and users' upgrade behavior are not, so the
tail after 1.3.0 is longer than "until we ship N+2".

### Equivalence re-established against the CURRENT artifact

PSLR-uxklngwq's 2026-08-07 proof compared 10,212 rules. The bundled snapshot
has since moved to 10,323 (upstream `9186eeed` -> `46ae48ce`), so that proof
no longer certified what is actually shipping. Re-run today against the current
index, rebuilding from the same bundled source under each punycoder:

    punycoder 1.2.1      (-v1 / 16.0.0)   10323 rows
    punycoder 1.2.1.9000 (-v2 / 17.0.0)   10323 rows

    identical(dim)     TRUE      10323 x 7
    differing cells    0  in line, raw, section, kind, canonical_rule,
                          canonical_key, labels
    identical(a, b)    TRUE

So the rebuild remains provably pure cost for this data -- but note that claim
had gone stale once already, silently, because it was pinned to a rule count
that a snapshot refresh moved. Any future use of it needs re-deriving against
the artifact actually being shipped, not inherited from an older comment.

### Consequence for this issue

Unchanged: still blocked on punycoder 1.3.0 being live, still the third step.
What changes is that the ordering is now evidence-backed rather than argued,
and the counter-argument has been tried and rejected on measurement.

#### 2026-09-09 — bartek@turczynski.pl

THIS ISSUE NOW HAS A DEADLINE. It did not before, and the difference is
measurable.

The reship was framed as retiring a per-session cost -- unpleasant but
open-ended. Measured 2026-09-09, that cost also shows up in `R CMD check`.
Same pslr 1.2.0 tarball, same machine, only the installed punycoder differing:

    example              vs punycoder 1.2.1   vs 1.2.1.9000
    psl_diff                       0.15 s          30.14 s
    psl_use                        0.03 s          10.32 s
    is_public_suffix               0.31 s           9.55 s
    psl_engine                     0.05 s           9.11 s
    total                          0.7  s          59.2  s

Exactly the four examples that activate a snapshot, and an 85x increase overall.
Against CRAN punycoder the check is clean; against the unreleased one it is

    * checking examples ... NOTE
    Examples with CPU (user + system) or elapsed time > 5s

So the sequence has a consequence nobody had costed: **the moment punycoder
1.3.0 goes live on CRAN, the published pslr 1.2.0 starts drawing an
examples-too-slow NOTE on CRAN's own check machines**, without pslr changing at
all. That is not an ERROR and does not block punycoder's submission, but it is
the kind of thing CRAN writes to maintainers about.

Consequence for planning: this issue should be prepared BEFORE punycoder 1.3.0
is submitted and held ready to submit immediately after 1.3.0 is live, rather
than started once 1.3.0 lands. The ordering constraint in the body is unchanged
-- do not reship until 1.3.0 is on CRAN -- but the gap either side of it should
be as short as it can be made.

Also worth noting for whoever does the reship: the equivalence claim this issue
leans on is now re-established against the current artifact. Rebuilding from the
bundled source under each punycoder gives 10,323 x 7 in both cases,
`identical()` TRUE, 0 differing cells. The older 10,212-rule proof had gone
stale when the snapshot refreshed, silently.
...
## PSLR-fqdepqem: Retire the Claude auto-memory store: promote what is durable, delete the rest
...
| Fact files | 3 |
| Size | 5 KB |
| MEMORY.md index | ~128 tokens, injected into every session while memory was on |
| Sessions in this project | 10 |
| Writes (Write/Edit on a memory file) | 3 |
| Explicit reads | 1 |
| Reads in a session that did NOT also write the file | **0** |

Store: `/Users/bartturczynski/.claude/projects/-Users-bartturczynski-Projects-pslr/memory`

Fleet-wide the ratio was 774 writes to 60 genuine reads, and 93.7% of files were never
genuinely read. Small store — likely a short skim-and-delete rather than a promotion pass.
...
```sh
tar -czf ~/Projects/_backups/claude-memory/pslr-memory-$(date +%F).tar.gz \
    -C "$(dirname '/Users/bartturczynski/.claude/projects/-Users-bartturczynski-Projects-pslr/memory')" memory
```
...
Store retired. Archived first to
`~/Projects/_backups/claude-memory/pslr-memory-2026-09-05.tar.gz` (3,327 bytes,
all four files verified present in the tarball), then deleted.

Four files, triaged into the issue's four buckets. Every claim was checked
against the tree before promoting or discarding it, and one turned out to be
actively harmful rather than merely stale.

**`email-identity-policy.md` — bucket 1, FALSIFIED, deleted.** It instructs:
"Git commits / pushes to GitHub -> use the GitHub noreply address
`142225707+bart-turczynski@users.noreply.github.com` (set repo-local
`user.email`)." That is now the opposite of policy. `8a0dc2d` deliberately set
`git config user.email` to `bartek@turczynski.pl` precisely because the noreply
address belonged to the suspended account and every new commit was being
authored to it; a `.mailmap` (present, 691 bytes) corrects the reading of the
152 historical commits carrying it. Following this memory would have re-broken a
fix that was made on purpose — the sharpest example of why the issue said to
verify before promoting. Its one durable half, that the DESCRIPTION maintainer
address is the plain `bartek@turczynski.pl` and the `+pslr` plus-tag is retired,
is already recorded in `cran-comments.md`, which documents that exact
incoming-feasibility NOTE. Nothing to promote.

**`MEMORY.md` — bucket 1, deleted.** Index of the other three.

**`pslr-ci-missing-secrets.md` — bucket 2/3, already covered, deleted.** Its
substance — which secrets exist, which jobs fail by design without them, and why
failing is correct rather than a bug — is already in `docs/git-workflow.md`'s
"The remote pipeline" section. Its README consequence is moot and was decided
its way: the memory argued "leaving both badges out is the honest choice", and
`README.md` today carries no GitLab pipeline or coverage badge. Verified, not
assumed.

**`pslr-local-gitlab-runner.md` — bucket 3, PROMOTED.** The only file holding
durable, project-specific knowledge the repository did not already carry.
`grep` for `gitlab-runner`, `local runner`, `54981995` and `project runner`
across `docs/`, `AGENTS.md`, `CONTRIBUTING.md` and `.gitlab-ci.yml` returned
nothing. Now a new subsection in `docs/git-workflow.md`, "Running the hosted
pipeline with no compute minutes": that a project runner is not subject to the
namespace quota block (established by running the full thirteen-job pipeline on
one while the quota was exhausted), what the runner is, that it is
single-concurrency, and that `pages` declares `needs: []` so `DEPLOY_PAGES=1` is
the right pipeline to raise for the site alone.

That file also carried an unfiled defect — "That hardcoded `-amd64.deb` is a
latent portability bug in the committed CI config, invisible on GitLab's x86
fleet ... not yet filed." Re-verified against the tree (three literals), filed as
**PSLR-wcjcywea**, fixed and merged this session (MR !19). So the memory's most
operationally painful content is retired rather than transcribed: the manual
per-tag digest pinning it prescribed is no longer needed. The note survives in
the docs only as the reason the class of bug exists — one a local runner exposes
and the hosted fleet hides.

Measured outcome: 4 files in, 1 promoted, 3 deleted, 1 new defect filed and
fixed. Consistent with the fleet-wide pattern the issue cites (93.7% never
genuinely read) — this store's own numbers were 3 writes, 0 reads in a session
that did not also write the file — but the residue was not nothing.
...
Resolved as option (b), settled by a three-way vote (this session's analysis, Codex gpt-5.6-sol high, an independent Claude reviewer): 2-1 for (b) over (a). Codex argued for (a) — report the runtime profile — on the grounds that the load-time normalizer is the reproducibility-relevant fact. Overruled because psl_version() already reports exactly that, so (a) would have bought agreement between the two APIs by making one redundant while leaving an orphaned field on disk. (b) keeps both facts and labels each honestly.

Shipped in 6ea140f, merged as ac8b53f (MR !17).

What changed:
- psl_snapshots() column normalization_profile -> first_normalization_profile, documented as the profile in use when the bytes were first published to the preferred location, with an explicit pointer to psl_version() for the profile a query runs under. Breaking, so a NEWS bullet marked as such.
- psl_snapshots_provenance() no longer coalesces the field across storage locations. It is read from the same location that supplies path/size/integrity. Two locations holding identical bytes can have been described under different normalizers and both records are honest; merging them picked one arbitrarily under a false premise.
- The snapshots.R coalesce comment no longer claims immutability for fields that are not immutable; it now names the three fields the argument does cover and says why the normalization fields are excluded.
- psl_publish_snapshot_descriptor()'s rationale comment corrected: returning an existing descriptor unchanged is now justified for the right reason — the normalization fields are first-publication provenance and are deliberately NOT refreshed — rather than by asserting they cannot change.

Acceptance criteria:
- Contradiction impossible: covered by a new regression test that publishes under the real profile, mocks runtime_normalizer_meta() to a fake profile, and asserts psl_snapshots() reports the recorded value while psl_version() reports the runtime one.
- Second new test: republishing the same bytes under a different profile keeps the first recorded value.
- False premise removed from snapshots.R.
- NEWS.md bullet with the issue id.

Verification: tools/verify.sh standard on the branch — lint clean, 2035 pass / 0 fail / 0 skip (up from 2030; +5 assertions from the two new tests). man/psl_snapshots.Rd regenerated via devtools::document().

Unicode_version was already absent from the public inventory — only normalization_profile was a column — so the fix covers it at the descriptor-rationale level only.
...
#### 2026-09-06 — bartek@turczynski.pl

SUPERSEDED 2026-09-06 by SEOR-eqpdrqnl, in the `seor` project. Closing as done
in the "transferred, not shipped" sense: NO CODE CHANGED UNDER THIS ID in this
pass, and the remaining work continues under the successor named below.

Deliberately NOT rewritten in place. Three repositories carry parked work that
cites this id; they need to find a pointer here, not a silently changed order.

### What actually happened to its three targets

This epic's technical target, as written on 2026-07-18:

  pslr first -> punycoder 1.2.x second -> rurl 2.7.0 last

That ordering was CORRECT WHEN WRITTEN. rurl then carried `Remotes:` git pins
on unreleased siblings, and CRAN held pslr 1.0.1 and punycoder 1.1.0.

Verified against live CRAN 2026-09-06 by fetching the published DESCRIPTIONs:

  punycoder  1.2.1   LIVE
  pslr       1.1.1   LIVE
  rurl       1.2.0   (still the old release)

**The first two links have already shipped.** Phases P and U completed
2026-07-19. The epic title never changed, which is the entire reason it still
reads as an open constraint on the third.

The third target is also stale in its own terms: it names **rurl 2.7.0**, and
rurl is now at **3.0.0**.

### Why the order is now inverted

rurl 3.0.0's DESCRIPTION carries **no `Remotes:` field**, and its floors are
`punycoder (>= 1.2.1)` and `pslr (>= 1.1.1)` -- exactly the versions CRAN
already serves. rurl 3.0.0 is submittable today with no ordering requirement of
its own.

It is also the fleet's single unlock: pagerankr needs `rurl (>= 3.0.0)`,
sitemapr `(>= 2.1.0)`, robotstxtr `(>= 2.2.1)`, and those three are seor's
remaining blockers. One submission clears a four-deep chain. So rurl went from
LAST to FIRST.

### Why the successor lives in `seor`, not here

This was a fleet-level coordination artifact filed inside ONE MEMBER's tracker.
That is precisely why it drifted unseen for seven weeks while three repos parked
work on its superseded ordering. The successor sits at fleet level:

  SEOR-eqpdrqnl  epic: fleet CRAN release chain, eight packages
    SEOR-suhsaxjd  Phase 0: zero-code unblocks
    SEOR-kbsrmutw  Phase 1: ship rurl 3.0.0 (the keystone)
    SEOR-ojdqkkmj  Phase 2: raddr (parallel)
    SEOR-zfnbbujk  Phase 3: punycoder 1.3.0, then pslr (parallel)

Two-tier split going forward: SEOR-* owns fleet sequence, gates and submission;
each package's own PREFIX-* owns its code. Note fp cannot reference across
projects -- `fp issue show` from another project returns "not found" -- so these
cross-tier links are necessarily plain text.

The corrected order is also now recorded in rurl's own
`design/release-chain.md` (MR !105, merged 2026-09-06), which is the file
rurl's AGENTS.md points agents at for submission order. A corrected epic that
rurl's own doc contradicted would have bought nothing.
...
## PSLR-kwareoss: Phase R: gate, submit and verify rurl 3.0.0

**Status:** todo | **Parent:** hrpalwzopsmqmdiajnstktmwzmzbbrrp

### Description

Deliver rurl to CRAN after compatible punycoder and pslr versions are live.

REWRITTEN 2026-08-12. Every version number in the previous text was wrong and
the two halves disagreed with each other: the title said 2.8.0, the body said
2.7.0, and the owner has since ruled that **rurl is submitted as 3.0.0 with no
further version bumps**. See the comment for the full list of dead premises.

### What is already true, measured at rurl 751816a

- rurl's DESCRIPTION carries **no `Remotes:` field**. The "without Remotes" half
  of this phase is done; `Rscript tools/dependency-resolvability-gate.R` in rurl
  reports `0 remote(s)`.
- The floors are `punycoder (>= 1.2.0)` and `pslr (>= 1.1.0)`, and both are
  satisfiable from CRAN today (punycoder 1.2.1, pslr 1.1.1). rurl 3.0.0 already
  installs from CRAN alone.

### What is left

1. One blocker, not a version question: `tests/testthat/test-format-url.R` calls
   `punycoder::unicode_versions()`, which no RELEASED punycoder exports --
   RURL-ovgasmea. rurl's dependency-resolvability gate fails on it at both
   resolution artifacts.
2. Run the `cran` gate for rurl (this is R3, PSLR-uwcwkfwm).
3. Submit (R4, PSLR-lararika) and verify live (R5, PSLR-wzrevacl).

### Ordering

The pslr-then-punycoder-then-rurl order this phase was written under is
superseded. The current order is raddr -> punycoder -> pslr -> **rurl 3.0.0** ->
robotstxtr + pagerankr -> sitemapr, each live on CRAN before the next is
submitted.
...
Rewritten 2026-08-12 rather than closed: the WORK survives, every PREMISE did not.

Dead premises removed, each verified against the live tree rather than assumed:

1. VERSION. Title said 2.8.0, body said 2.7.0, and the two contradicted each other. Owner ruling: rurl ships as 3.0.0, no further bumps. Both numbers are now unreachable.

2. 'WITHOUT REMOTES' IS ALREADY DONE. rurl's DESCRIPTION at 751816a has no Remotes field at all. The phase was named after a task that has since been completed elsewhere.

3. THE CURL PREMISE. The R3 comments on the child issue diagnose Windows test failures as 'libcurl-version divergence' and state 'rurl is pure R, delegates URL parsing to the curl package's curl_parse_url()'. That is no longer true: curl is not a dependency and tools/curl-zero-gate.R proves the absence on every run. An agent scoping from that text would go looking for a seam that does not exist.

4. THE ORDERING. 'pslr first, punycoder second, rurl last' assumed neither sibling was on CRAN. Both are live (punycoder 1.2.1, pslr 1.1.1). The current order is raddr -> punycoder -> pslr -> rurl 3.0.0 -> robotstxtr + pagerankr -> sitemapr.

WHAT REPLACES THEM is one concrete blocker, now backed by an executable check rather than a narrative: RURL-ovgasmea. rurl's new tools/dependency-resolvability-gate.R fails on it at BOTH resolution artifacts (punycoder tag v1.2.0 and CRAN 1.2.1).

Not closed, and not moved to rurl's tracker either -- that is a structural question about where cross-repo release phases should live, and it is worth asking once for the whole chain rather than answering by unilateral drift here.




## PSLR-lararika: R4: submit the approved rurl 3.0.0 release to CRAN
...
REWRITTEN 2026-08-12: retargeted from 2.8.0/2.7.0 to 3.0.0. The two halves of
this issue previously disagreed -- the title said 2.8.0, the body said 2.7.0.

Submit only after R3 (PSLR-uwcwkfwm) is an unqualified `cran` go and the
official CRAN index still satisfies rurl's floors. Verify the exact merged
commit, source tarball, `DESCRIPTION` without `Remotes`, NEWS, cran-comments,
dependency floors, and tarball hash.

Submit exactly one rurl source tarball and record version, commit, hash, method,
time, and acknowledgement. Do not close on upload. On rejection, return to
scoped repair, rerun the CRAN-only dependency and `cran` gates affected by the
change, and complete this phase before downstream cleanup.

### Two things this phase must not be read as authorizing

1. **Do not submit before the backlogs are drained.** The owner has ruled that
   nothing is submitted until the repos are drained. This issue's original text
   said "the owner's orchestration approval authorizes this submission child",
   which was written before that ruling and must not be read as standing
   authority now.
2. **rurl is not first.** Order: raddr -> punycoder -> pslr -> **rurl 3.0.0** ->
   robotstxtr + pagerankr -> sitemapr, each live on CRAN before the next.

### Comments

#### 2026-08-12 — bartek@turczynski.pl

Retargeted 2026-08-12 to 3.0.0; title and body had disagreed (2.8.0 vs 2.7.0) since 2026-07-26.

Added a constraint that was NOT in the original and would otherwise have been missed: this issue carried a standing submission authorization ('the owner's orchestration approval authorizes this submission child'). Two owner decisions since then narrow it -- rurl ships as 3.0.0, and NOTHING is submitted until the backlogs are drained. An agent picking this up on its own terms would have found written authority to submit. It no longer has it, and the body now says so.

#### 2026-09-08 — bartek@turczynski.pl

Submitted 2026-09-08 20:57:18 UTC via devtools::submit_cran(). Maintainer received the CRAN confirmation email and clicked the link, so the submission is queued, not merely uploaded.

Two deviations from this issue title, both deliberate and both already recorded upstream:

1. VERSION IS 3.0.1, NOT 3.0.0. 3.0.0 was submitted 2026-09-06 and came back 1 ERROR / 1 NOTE on r-devel-windows-x86_64 -- six failures in test-external-url-vectors.R from a Windows text-mode file connection dropping 3443 bytes mid-corpus. Fixed under RURL-gxgxyzpk (rurl 88377c7): read_vectors() now reads with readBin() and parses via read.csv(text = ), opening no connection at all. Verified green on win-builder R-devel as probe build 3.0.1.9002 (FAIL 0 | PASS 7157). The owner ruling of "no further version bumps" was made before that ERROR existed; 3.0.1 is the repair of a rejected 3.0.0, not a drift past the ruling.

2. SUBMITTED SHA IS 0ca8f096, one merge past the fix. That merge is rurl MR !109, a repository-hygiene change only: four gate logs under tmp/orchestrate/ untracked, tmp/ added to .gitignore, and one paragraph of cran-comments.md updated to record both halves of that defect as fixed. No executable content, and no effect on the tarball -- .Rbuildignore already excluded ^tmp$.

Gate at the submitted SHA: 22 blocking steps, 0 failed. R CMD check --as-cran at Status: 1 NOTE (CRAN incoming feasibility), all four components explained in cran-comments.md. Suite green under LC_ALL=C.

R5 (PSLR-wzrevacl) is now the live frontier: CRAN has not accepted anything yet.
...
## PSLR-lidgnomk: verify.sh reads OSSINDEX_TOKEN from the shell but tells you to put it in ~/.Renviron: the OSS Index audit never runs

**Status:** done | **Parent:** ugxanxnefzohafakgwjujmhmwjteyjku

### Description

`tools/verify.sh` gates the OSS Index audit on a **shell** environment variable,
but the credentials the gate's own error message tells you to set live in
`~/.Renviron`, which only R reads. The two never meet, so the audit is skipped
on every `full` run no matter how correctly the credentials are configured.

### Measured, 2026-09-05

Credentials are present and valid in `~/.Renviron`:

    $ Rscript -e 'cat(nchar(Sys.getenv("OSSINDEX_TOKEN")))'
    61

The shell that runs `tools/verify.sh` sees nothing:

    $ echo "[${OSSINDEX_TOKEN:-UNSET}]"
    [UNSET]

`run_security()` therefore takes its skip branch:
...
`~/.Renviron` is read by R at startup. It is not a shell file, nothing sources
it into the environment `verify.sh` runs in, and `verify.sh` does not read it.
So the guard's precondition can never be satisfied by following the guard's own
instruction.

### Why this matters more than a skipped step

This is PSLR-njeqwltb re-entering through a different door. That issue was filed
because the OSS Index audit was **vacuously green** — it passed while auditing
nothing. The fix added this guard so an unauthenticated audit is reported rather
than silently passed. The guard works; it is simply looking in the wrong place,
and the result is that the audit never runs at all on a correctly configured
machine. `full` is the tier that "replaces the weekly CI schedules", so the
dependency-advisory half of that replacement has not been running.

Note the R side would work if reached: `NOT_CRAN=true Rscript -e
'testthat::test_local(filter = "security", ...)'` starts an R process, which
reads `~/.Renviron` and sees the token. Only the bash guard in front of it is
wrong.

### The `cran` tier is affected differently, and worse

The `cran` tier makes the same test hard:
...
So the pre-submission tier aborts before doing anything, on a machine whose
credentials are correctly installed. Worth establishing how the 2026-09-05 cran
run (MR !16) got past this — whether the token was exported by hand in that
shell, in which case the tier has only ever passed by an undocumented manual
step.

### Fix

Read the value the same way R does rather than requiring it twice. Parse
`OSSINDEX_TOKEN` (and `OSSINDEX_USER`) out of `${R_ENVIRON_USER:-$HOME/.Renviron}`
when the shell variable is unset, and export it for the child R process, so that
one documented location works for both. Keep the shell variable as an override
so CI, which sets real environment variables, is unaffected.

Two details a fix must respect:
  - `.Renviron` lines may be quoted and may contain `=` in the value; take
    everything after the first `=` and strip one layer of surrounding quotes.
  - The token must not be echoed. It is a live credential; do not print it, and
    do not let `set -x` or an error path leak it.

### Done when

- With credentials only in `~/.Renviron` and nothing exported, `tools/verify.sh
  full` actually runs the OSS Index audit instead of warning and skipping.
- With credentials absent from both places, the `full` warning and the `cran`
  hard failure still fire — the guard must not be weakened into the vacuous pass
  PSLR-njeqwltb removed.
- An exported shell variable still wins over the file.
- The warning text no longer instructs the user to do something that does not work.

### Provenance

Found while verifying PSLR-ugxanxne's remaining checkbox ("OSSINDEX_USER /
OSSINDEX_TOKEN in ~/.Renviron so the full tier is not vacuous"). The credentials
were placed as instructed; the checkbox is nonetheless unmet, and could not have
been met that way.
...
Landed in MR !22, commit 7413bf2, merged to main.

The guard now resolves the credentials the way R does — from
`${R_ENVIRON_USER:-$HOME/.Renviron}`, not a hardcoded path — when and only
when the environment does not already define them, and exports what it finds
for the child R process. One shared helper serves both the `full` soft gate and
the `cran` hard gate, so they cannot drift apart again.

Verified by execution rather than from the report, against the real machine and
against fixtures:

  1. file only, nothing exported -> resolves, token length 61, user length 1,
     matching R's own `nchar`
  2. exported value wins over a different value in the file
  3. no file at all -> does not resolve and does not abort under `set -euo pipefail`
  4. `bash -x` trace contains zero occurrences of the credential

Containment, since this handles a live token: `source` is not used on
`.Renviron` (it is data, not a shell script); assignment is `printf -v` so the
value never transits a command substitution or an echo; `xtrace` is suspended
across the read and restored after; presence is tested with `${VAR:+set}` rather
than `${VAR:-}` — not cosmetic, the first draft used the plain expansion and a
trace canary caught the token appearing in `+ '[' -n <token> ']'`. Both messages
name the path, never the value.

Tier semantics deliberately unchanged: `full` still warns softly and returns 0
with credentials absent from both places, `cran` still fails hard and exits 1.
PSLR-njeqwltb's vacuous pass does not come back. The warning text no longer
instructs an action that cannot work.

Gate: `tools/verify.sh standard` — lint clean, 2035 tests, 0 failures, identical
to the `main` baseline. `bash -n` clean; `shellcheck -S warning` reports nothing
in the new code. `--staleness` still prints one line and exits 0, so the hook
stays safe. No NEWS bullet: `tools/verify.sh` is not in the tarball.

Left open deliberately, on PSLR-ugxanxne rather than here: nothing in the tree
explains how the 2026-09-05 `cran` run got past the old hard guard, which has
existed since 8ada982 and sits before every step of that tier. Either the token
was exported ad hoc in that shell — in which case the audit did run that day and
the outage was narrower than assumed — or that run was not a literal end-to-end
`tools/verify.sh cran`. A `full` run now is the confirmation that the audit
actually executes.
...
## PSLR-qjsszyig: A bundled-snapshot bump leaves README.md advertising the previous snapshot; nothing in the bump procedure regenerates it

**Status:** done

### Description

A bundled-snapshot bump silently invalidates `README.md`, and nothing in the bump
procedure regenerates it. Found when the first `tools/verify.sh full` run after
`799964f` failed its README drift check.

### What happened

`README.md` embeds live `psl_status()` output, which prints the active
snapshot's checksum, content date and retrieval time. `799964f` moved the
bundled snapshot from 9186eeed to 46ae48ce, so all three lines changed — and the
README was not regenerated. Between that commit and the fix, `README.md`
advertised the provenance of a snapshot the package no longer bundles:

    checksum:     sha256:54fb5c65a1e2...      actual 00dda6fa8406...
    content date: 2026-06-13 21:47 UTC        actual 2026-09-05 14:17 UTC
    retrieved:    2026-07-26 13:00 UTC        actual 2026-09-05 14:40 UTC

Regenerated in the commit that cites this issue's sibling PSLR-ucvugmsw. This
issue is the process gap, not that one file.

### Why nothing caught it

Verified against the tree rather than assumed:

  - `data-raw/update_psl.R` and `data-raw/snapshot-tracker.sh` contain no
    `build_readme()` call and no README step at all.
  - The README drift check exists only in `tools/verify.sh` **`full`**.
  - The pre-push hook runs `standard`, which deliberately excludes it.
  - The hosted `readme` job runs only under `CRAN_PREP=1`, which nobody raises
    for a routine snapshot bump.

So the only thing that can catch this is a `full` run, which is nudged weekly by
the staleness line rather than required. The bump landed at 14:40 UTC; the
previous successful `full` was recorded at 12:26 local the same day, i.e.
*before* it. The window was structural, not bad luck.

### Why it is more than cosmetic

D17 records that a bundled-snapshot change **is a released version**. The README
is the package's front page on the repository and on the pkgdown site, and those
three lines are the package's own provenance claim about the data it ships.
Publishing a checksum that does not match the shipped bytes is the same class of
defect as PSLR-girwpagy (an inventory column naming a normalization profile that
was no longer in use) — two surfaces disagreeing about the same artifact, where
the wrong one looks authoritative.

It is also self-concealing: the drift is invisible in `git diff` at bump time
because nothing regenerates the file to compare against.

### Options

1. **Regenerate in the bump pipeline.** Add `devtools::build_readme()` to
   `data-raw/update_psl.R` after the artifacts are written, so the README moves
   with the bytes it describes and the drift cannot be committed. Cheapest and
   closes the window entirely — but the pipeline then depends on pandoc, and the
   pinned pandoc question the `readme` job documents (distro pandoc pads pipe
   tables) applies to whoever runs it locally.
2. **Add the drift check to `standard`.** Correct but probably wrong: the tier
   is deliberately kept at ~2 minutes, and `build_readme()` is not cheap.
3. **Add it to the snapshot-bump checklist** (PSLR-rwrzaqsv wired
   bundled-snapshot regeneration into the pre-CRAN checklist already, so there
   is a place for it). Weakest — a checklist step is exactly what was missed.

Option 1 is preferred: the failure mode is "two artifacts that must agree were
generated at different times", and the fix for that is to generate them
together, not to remember to.

### Done when

A bundled-snapshot regeneration cannot leave `README.md` describing the previous
snapshot — demonstrated by running the bump pipeline on a fresh upstream commit
and seeing `README.md` change in the same working tree, with no separate step.
...
Landed in MR !24, commit 5fa9c1c, merged to main.

`data-raw/update_psl.R` now regenerates `README.md` alongside the four data
artifacts it already writes, so the document moves in the same working tree as
the bytes it describes. Option (a) from the description.

Guarded, and it never aborts — the data artifacts are written and reported
before this runs, so a rendering problem must not bury a correct snapshot report
under an unrelated failure. Every path that cannot render prints
`README.md is now stale — run devtools::build_readme() and commit it` plus the
specific reason, and exits 0:

  - pandoc not on PATH
  - `devtools`, `rmarkdown` or `callr` missing
  - local pandoc differs from the `PANDOC_VERSION` pinned in `.gitlab-ci.yml`
  - `build_readme()` itself throws

One refinement over what this issue proposed, found in live code and verified
independently: `requireNamespace("devtools")` alone is NOT a sufficient check.
`rmarkdown` and `callr` are *Suggests* of devtools, not Imports — confirmed
against the installed DESCRIPTION — and `devtools:::build_rmd_impl()` reaches
them via `check_installed("rmarkdown")` and `callr::r_safe()`. All three are
checked, and the message names whichever are absent.

The version pin is read from `.gitlab-ci.yml` with the same pattern
`tools/verify.sh`'s `run_readme` uses, so the two cannot drift. Where
`run_readme` only warns on a skew — it renders and then diffs — this blocks the
render, because here the output lands in the working tree, and a whitespace-only
diff that has to be told apart from a real content change by hand is worse than
a message naming the version to install.

Verified: five guard states exercised in isolation with a PATH-shimmed pandoc
and mocked package availability, no network and no real render; all exit 0.
Inverting the version comparison makes the skew case render when it must warn,
so the guard bites. On this machine the pin reads 3.10, local pandoc reads 3.10,
and the blocker is NULL. `data-raw/` is `.Rbuildignore`d, so nothing here reaches
the tarball.

Gate: `tools/verify.sh standard` — lint clean, 2035 tests, 0 failures. The first
run failed lint (`paste_linter` wanting `toString()`); fixed to the house idiom
`R/diff.R:137` already uses, and re-run green.

### The decision, and the part left open

The panel split 2–2 rather than reaching a majority. Two voters ruled for option
(a) as implemented; two ruled that the real defect is the README embedding
volatile provenance at all, and would have removed the checksum and timestamps
instead.

Broken toward (a) on evidence: `README.Rmd` already carries a `freshness-setup`
chunk whose stated purpose is making that block byte-identical across machines,
so the values are not render-volatile and the only input that moves them is a
snapshot bump — which D17 records as a released version. The dissent's premise
that these are volatile front-page decoration does not survive that. It should
also be recorded that the panel prompt overstated the pandoc hazard: `run_readme`
already renders with local pandoc and demands byte-identical output, so (a) adds
no hazard the gate did not already impose. That framing error plausibly moved
both dissenting votes, which is a reason to hold the tiebreak loosely.

The dissent is filed as **PSLR-wydqlubg**, not discarded — including the one
point all four voters agreed on, that `retrieved:` (the wall-clock second the
pipeline ran) has no reader value. This fix stays correct whichever way that
goes, and is cheap to revert if the maintainer prefers the other reading.
...
## PSLR-rnfnzwqu: Make the profile-rebuild and dedup tests agnostic to the installed punycoder profile

**Status:** done

### Description

Two test families in pslr assert against the *no-rebuild* path rather than
against the behavior they are named for, so they fail whenever the installed
punycoder's normalization profile differs from the one the bundled index was
generated under. That makes pslr un-releasable in either direction across a
punycoder profile move, and it is currently the blocker on punycoder 1.3.0.

### Measured, 2026-09-09

`R CMD check` on the CRAN tarball `pslr_1.1.1.tar.gz` against punycoder
1.2.1.9000 (Unicode 17.0.0, `uts46-nontransitional-std3-v2`):

    Status: 1 ERROR
    [ FAIL 4 | WARN 0 | SKIP 2 | PASS 666 ]

    test-profile-rebuild.R:11 -- `the_matcher$state$snapshot$rebuilt`
                                 expected FALSE, got TRUE
    test-dedup.R:53           -- `res$norm` expected 1,  got 10213
    test-dedup.R:64           -- `res$norm` expected 3,  got 10215
    test-dedup.R:77           -- `res$norm` expected 3,  got 10215

Nothing is wrong with the package. `bundled_snapshot()` (`R/matcher.R:209`)
compares both `normalization_profile` and `unicode_version` against the runtime
normalizer and rebuilds the index in memory on either mismatch -- the
documented, correct behavior, and PSLR-uxklngwq's 2026-08-07 element-wise diff
already proved the rebuilt rule table is content-identical (10,212 rules, zero
differing cells). The failures are the tests measuring the wrong thing.

### What each test actually asserts vs. what it means to assert

**`test-profile-rebuild.R:8-12`** -- "a matching profile uses the shipped index
without rebuilding" hardcodes `expect_false(...$rebuilt)`. That is true only
when the bundled index happens to have been generated under the same profile as
the punycoder installed at check time. It is an assertion about a build-time
coincidence, not about the contract. The contract is: `rebuilt` is TRUE exactly
when the bundled meta and the runtime meta disagree. Derive the expectation
from `pslr_bundled$meta` vs `runtime_normalizer_meta()` and the test asserts the
real invariant and holds under every profile.

**`test-dedup.R:49,60,73`** -- these measure normalizer and matcher crossings to
prove a repeated host costs one normalization. Each calls `local_pslr_clean()`
and then wraps the FIRST `public_suffix()` call in `count_crossings()`, so the
counted region contains one-time activation as well as the query. When
activation rebuilds, its 10,212 source-rule normalizations land in the count.
The tests are about per-query dedup, not activation, so activation belongs
outside the measured region -- `psl_use("bundled")` before `count_crossings()`.
That is a strict improvement independent of the profile question.

### Why this is on the critical path

Circular gate, recorded in full on punycoder PUNY-tjusxhgn:

  - PUNY-tjusxhgn: do not submit punycoder 1.3.0 until its reverse dependency
    passes against it. pslr 1.1.1 is live on CRAN and fails as above, so CRAN's
    incoming revdep check would see a hard ERROR.
  - PSLR-fjkaqckg: do not reship pslr's index under 17.0.0 until punycoder
    1.3.0 is live -- doing it earlier inverts the rebuild cost onto every user
    still on 1.2.1, and pslr's own CRAN check (run against punycoder 1.2.1)
    would fail the mirror image of these same four failures.

Fixing the tests breaks the cycle without reshipping anything: a pslr release
carrying profile-agnostic tests passes under punycoder 1.2.1 AND 1.3.0.

### Scope

1. `test-profile-rebuild.R` -- derive the expected `rebuilt` from the
   bundled-vs-runtime meta comparison instead of hardcoding FALSE. Keep the
   existing mocked-mismatch tests as they are; they already pin the TRUE side.
2. `test-dedup.R` -- activate before measuring, in all three tests.
3. Verify green against BOTH punycoder populations: the installed 1.2.1.9000
   (17.0.0 / `-v2`) and CRAN 1.2.1 (16.0.0 / `-v1`) in a throwaway library.
   Running against only one proves nothing here -- that is the whole defect.

### Out of scope

Dropping the `Remotes:` dev pin (`DESCRIPTION:44`) and raising the floor to
`punycoder (>= 1.3.0)` are release actions, PSLR-uxklngwq step 4. Reshipping
`R/sysdata.rda` under 17.0.0 is PSLR-fjkaqckg and still trails punycoder 1.3.0
going live.
...
FIX BUILT AND VERIFIED AT RELEASE-CHECK LEVEL (2026-09-09). Branch
`fix/profile-agnostic-tests`, two commits, not yet pushed.

### The mirror direction is now measured, not inferred

The issue body asserted that shipping a `-v2` index against CRAN punycoder
1.2.1 fails the mirror image of the same four assertions. That was an
inference. Measured today on `main` in a throwaway worktree, against punycoder
1.2.1:

    test-profile-rebuild.R:11   rebuilt   expected FALSE, got TRUE
    test-dedup.R:53             res$norm  expected 1,     got 10326
    test-dedup.R:64             res$norm  expected 3,     got 10326
    test-dedup.R:77             res$norm  expected 3,     got 10326

Same four, opposite direction. (10326 rather than 10213/10215 because the dev
tree's snapshot carries 10,323 rules against 1.1.1's 10,212.) The circularity
recorded on punycoder PUNY-tjusxhgn is therefore real in both directions.

### The fix, and what it was verified against

`test-profile-rebuild.R` derives the expected `rebuilt` from the same
bundled-versus-runtime comparison `bundled_snapshot()` makes. `test-dedup.R`
activates before measuring, so one-time activation is outside the counted
region.

Verified against both punycoder populations, since running against one is the
defect itself:

  - full suite under installed 1.2.1.9000 (17.0.0 / `-v2`, no rebuild, derived
    expectation FALSE) -- green
  - full suite under CRAN 1.2.1 in a throwaway library (16.0.0 / `-v1`,
    rebuild fires, derived expectation TRUE) -- green

Both branches of the derivation exercised, so the derivation is not vacuous.

And at the level that actually matters for submission -- `R CMD build` then
`R CMD check --as-cran` on the resulting tarball, resolved against punycoder
1.2.1:

    Status: 1 NOTE      [ FAIL 0 | WARN 0 | SKIP 2 | PASS 2033 ]

That is the exact condition CRAN would run pslr in today, and it passes. See
the separate finding below for the NOTE.

### Two findings that are NOT this issue

1. **`tmp/` is missing from `.Rbuildignore`.** It is gitignored but not build
   ignored, so `R CMD build` from a working tree carries it into the tarball
   and `R CMD check` reports `Non-standard file/directory found at top level:
   'tmp'`. That is the entire content of the NOTE above -- a clean-clone build
   would not hit it, but any build from a working tree that has run the gate
   will. rurl hit the same class of thing at `6a18dda`. One line to fix, filed
   separately rather than smuggled into this branch.

2. **The pre-push verify gate is red on `test-security.R:23`, unrelated to this
   branch.** `DESCRIPTION` is byte-identical to `main`, and the audit's only
   input is `DESCRIPTION`. OSS Index reports a vulnerability in the installed
   `curl`: 1 against 7.1.0, and 2 against 8.0.0, which is what CRAN currently
   serves -- CVE-2026-18924 (libcurl HTTP/2 server-push use-after-free) and
   CVE-2026-3783 (OAuth2 bearer token leaked across a redirect via `.netrc`).
   No `curl` on CRAN clears the audit, so this stays red. It skips under
   `R CMD check` via `skip_on_cran()`, so CRAN and the `--as-cran` tier are
   unaffected; it blocks only the local pre-push hook, and only on a machine
   where `OSSINDEX_USER` / `OSSINDEX_TOKEN` are set. Awaiting a maintainer
   decision; the branch is committed and unpushed until then.

#### 2026-09-09 — bartek@turczynski.pl

DONE. Merged as MR !26 (merge commit 8e36709), branch deleted local and remote.

Three commits: the test fix (6083b0b), an 80-column lint follow-up (47e1937),
and `^tmp$` added to `.Rbuildignore` (83f6411) for the one NOTE the release
check surfaced -- rebuilding after it yields 0 paths under `pslr/tmp/`.

Pushed with `--no-verify` on explicit maintainer approval. The gate's lint and
`--as-cran` tiers passed; it was red only on `test-security.R:23`, unrelated to
this branch. That finding is real -- the CRAN `curl` binary statically links
libcurl 8.14.1, which CVE-2026-18924 and CVE-2026-3783 both cover -- but is not
reachable through pslr's single download path, and libcurl 8.22.0 (2026-09-02)
fixes both, so it clears on `curl`'s next CRAN build. The gate's contract is
now tracked fleet-wide as SEOR-fkvlzltx in the seor backlog, since seven of the
eight repositories carry the same test.

What this unblocks: pslr can now be released against either punycoder profile,
which is step 1 of the corrected three-step order recorded on PSLR-fjkaqckg.
Still open there and not touched here: reverting `R/sysdata.rda` to `-v1`,
dropping the `Remotes:` pin, and the `cran-comments.md` rewrite. Maintainer has
set the next version at 1.2.0.
...
Maintainer decision: option (b). GitLab becomes the declared home for pslr, including the URLs CRAN advertises. GitHub restoration is not being pursued.

Done so far: project 85027051 flipped visibility private -> public and pages_access_level private -> public. https://gitlab.com/bart-turczynski/pslr and /-/issues now resolve anonymously. Pre-flight before flipping: no secrets in working tree or history (filename + pattern scan), no CI job echoes a token value, _scratch/ and .fp/ untracked.

Next: ungate the pages job, sweep the remaining GitHub URLs (.bestpractices.json, README badges), declare the pkgdown site in DESCRIPTION.

#### 2026-08-09 — bartek@turczynski.pl

Merged !12 (commit 1c25189). Landed: pages job ungated; `api` added to the workflow allow-list so the site can be rebuilt with `glab ci run` between releases; DESCRIPTION declares the pkgdown site as homepage; .bestpractices.json swept to GitLab and three stale claims corrected (rhub -> tools/verify.sh sanitize, Codecov -> CI Cobertura, GitHub advisories -> maintainer email) plus the CI-frequency claims corrected to match the narrowed workflow rules; dead Codecov badge replaced with a docs-site badge; cran-comments.md documents the BugReports 404.

Two findings worth recording:

1. Pages was defaulting to a UNIQUE DOMAIN (https://pslr-2af360.gitlab.io), not https://bart-turczynski.gitlab.io/pslr/ which _pkgdown.yml and DESCRIPTION declare. Fixed via PATCH /projects/85027051/pages pages_unique_domain_enabled=false. The API now reports the expected URL.

2. BLOCKED on deploy: the namespace compute quota is exhausted. Pipeline 2745523194 on main was created (proving the api workflow rule works) but every one of its 13 jobs failed with ci_quota_exceeded, pages included. GitLab.com has no API to upload a Pages site directly (POST /projects/:id/pages/deployments -> 404), so publishing needs a runner. Options are quota reset (1st of month), the GitLab for Open Source programme (now that the project is public), buying minutes, or a self-hosted runner — Docker is available locally, gitlab-runner is not installed. Maintainer decision.

Consequence to watch: DESCRIPTION now advertises the pkgdown URL, which 404s until a deploy succeeds. Must not be submitted to CRAN before then.

#### 2026-09-05 — bartek@turczynski.pl

Status check 2026-09-05, measured rather than assumed. The blocker this issue was filed for is resolved; what remains is one thing only, and it is downstream of a release.

VERIFIED RESOLVED — every declared URL resolves anonymously:

    200  https://bart-turczynski.gitlab.io/pslr/          DESCRIPTION URL (pkgdown)
    200  https://gitlab.com/bart-turczynski/pslr          DESCRIPTION URL (repo)
    200  https://CRAN.R-project.org/package=pslr
    200  https://bart-turczynski.r-universe.dev

The project is `visibility: public` with `pages_access_level: public`, so option (b) is complete for pslr. `https://gitlab.com/bart-turczynski/pslr/-/issues` returns 404 to a logged-out client, but so does `https://gitlab.com/gitlab-org/gitlab/-/issues` — that is GitLab's site-wide behaviour for anonymous issue lists, not a defect in this project. Measured against that peer in the same run rather than read as a finding.

GITHUB NOW FULLY RETIRED FROM THE REPO (8a0dc2d, MR !15). The load-bearing remnant was not a link at all: `git config user.email` was still `142225707+bart-turczynski@users.noreply.github.com`, so every new commit was authored to the suspended account. Set to bartek@turczynski.pl. A `.mailmap` corrects the reading of the 152 commits already carrying that address, plus 20 on a former employer's address, without rewriting published history — `git shortlog` now reports 172 commits under the real identity. `.Rbuildignore`'s `^\.github$` removed with the directory it ignored.

The 30-odd remaining `github.com` hits in the tree were each checked and all are third-party or not hosts at all: pandoc's release downloads, `r-lib.github.io` for pak, the `publicsuffix/list` upstream API (the PSL itself lives on GitHub), ACKNOWLEDGMENTS/CoC/pre-commit links, and a benchmark fixture string. `README.Rmd`'s `github_document:` is an rmarkdown output format, not a host — changing it would break the CI README diff. `.fossa.yml`'s `project.id` keeps its `git+github.com/...` locator for the reason the file already documents at length: it is FOSSA's primary key, both README badges resolve 200 today, and repointing it registers a second empty project rather than moving the existing one.

STILL OPEN, and only a release closes it: CRAN's live page for pslr 1.1.1 still advertises the dead metadata. Read from crandb today:

    URL:        https://bart-turczynski.github.io/pslr/, https://github.com/bart-turczynski/pslr, ...
    BugReports: https://github.com/bart-turczynski/pslr/issues

Every fix so far is local. Anyone arriving from the CRAN page still hits 404s, and the acceptance criterion "BugReports points somewhere that accepts a bug report from a stranger" is unmet for the published version. This is the last open item on this issue and it is a decision: patch-release pslr to correct the live metadata, or let it ride to the next release. Given the release chain is already queued, riding is defensible — but it should be a decision on the record rather than a default.

#### 2026-09-05 — bartek@turczynski.pl

New evidence for the open decision on pslr 1.1.1's live CRAN metadata, from the 2026-09-05 cran-tier run (MR !16).

Once the local as-cran check was fixed to actually run to completion (PSLR-zibafvbq had it aborting at its first step), the URL check flagged the CURRENT, already-corrected BugReports value:
...
Measured anonymously in the same run against a known-good peer: https://gitlab.com/gitlab-org/gitlab/-/issues also returns 404, while https://gitlab.com/bart-turczynski/pslr and https://bart-turczynski.gitlab.io/pslr/ both return 200. So this is GitLab's site-wide anti-scraping behaviour for logged-out clients, not a defect in this project — the existing constraint on that stands.

What is new is the submission consequence. CRAN's --as-cran URL check runs anonymously, so ANY pslr submission will carry this NOTE regardless of which decision is taken here, and a reviewer may query it. That is worth a line in cran-comments.md (the item still open from PSLR-totktvlq) whichever way the patch-release-vs-ride decision goes.

It also sharpens this issue's acceptance criterion. 'BugReports points somewhere that accepts a bug report from a stranger' is unmet for the published 1.1.1 because the GitHub tracker is gone; but even after the fix ships, an anonymous visitor following the GitLab BugReports URL gets a 404 page rather than a form. Worth deciding whether that satisfies the criterion, or whether the tracker needs to be reachable to logged-out users.

#### 2026-09-05 — bartek@turczynski.pl

Decision recorded: RIDE — CRAN 1.1.1's metadata is left as-is; the corrected GitLab URLs ship with the next release when the queued chain reaches pslr. Not a default; settled by a three-way vote (this session's analysis, Codex gpt-5.6-sol high, and an independent Claude reviewer) — unanimous.

Rationale, in the order it weighed:
1. A patch release does NOT satisfy this issue's acceptance criterion. GitLab 404s /-/issues for anonymous clients site-wide, and CRAN's URL checker is anonymous, so a logged-out stranger following the corrected BugReports URL still lands on a 404 page rather than a bug form. PATCH would spend a CRAN submission and still leave the criterion unmet.
2. DESCRIPTION carries Remotes:, a hard CRAN blocker that cannot be removed until punycoder 1.3.0 is live. A patch now would have to specially work around it, deviating from the queued chain.
3. What a patch would genuinely fix — reachability of the source — is partly mitigated: the corrected repo root and pkgdown site both return 200, and the package stays installable from CRAN meanwhile.

Follow-on, still open on this issue: cran-comments.md needs a line explaining the anonymous BugReports 404, because EVERY future pslr submission carries that NOTE regardless of this decision.

#### 2026-09-05 — bartek@turczynski.pl

Closing this. The last open follow-on was already shipped when it was recorded as
outstanding, and I verified it rather than taking the note at face value.

The 2026-09-05 comment above ends: "Follow-on, still open on this issue:
cran-comments.md needs a line explaining the anonymous BugReports 404, because
EVERY future pslr submission carries that NOTE regardless of this decision."

That line exists on `main` and has since **2026-08-09**, in `1c25189` — 27 days
before the comment that listed it as open. `git log -- cran-comments.md` shows
`1c25189` as the newest commit touching the file, and `git show
main:cran-comments.md` carries a full section headed "Expected additional NOTE
from the next submission onwards" which does more than the follow-on asked for:
it quotes the exact `URL: ... Status: 404` block a reviewer will see, states the
false-positive diagnosis, and names `https://gitlab.com/gitlab-org/gitlab/-/issues`
as the known-good peer measured in the same run — the same peer-comparison
discipline the rest of this issue used.

So every acceptance criterion is either met or consciously deferred by the
recorded RIDE decision:

  - Every declared URL resolves (200 for repo root, pkgdown site, CRAN page,
    r-universe) — verified in the 2026-09-05 comment.
  - BugReports points at the GitLab tracker; the anonymous 404 is GitLab
    site-wide behaviour, established against a peer, and is now documented for
    CRAN in cran-comments.md.
  - README badges and _pkgdown.yml links resolve.
  - The invalid-URL NOTE will still appear (it is anonymous-checker behaviour
    nothing in the package can change) and is pre-explained to the reviewer.
  - The live CRAN 1.1.1 metadata question was settled as RIDE by unanimous
    three-way vote, on the record.

Nothing is left that is not a CRAN submission, which belongs to the release
chain (PSLR-hrpalwzo), not here. No code changed under this id in this session.
...
Scope reconciliation 2026-09-05 — the three items left open in this issue are all now closed or deliberately superseded.

Item 3 (pkgdown hosting) — DONE. `pages_access_level` is `public` and https://bart-turczynski.gitlab.io/pslr/ serves 200. The `PKGDOWN_PAGES` gate this issue asked about is gone; the job is ordinary, and now runs on `DEPLOY_PAGES=1` or as part of CRAN prep.

Item 5 (fate of `.github/`) — DONE, ahead of the recommendation in this issue's body. The nine dead workflows were removed in d8f9244 (PSLR-bbuuafin) rather than kept pending a GitHub appeal. The last trace, `^\.github$` in `.Rbuildignore`, went in 8a0dc2d.

Item 2 (four weekly schedules) — SUPERSEDED and now enforced. PSLR-ugxanxne replaced them with the local `full` tier; as of 8a0dc2d the `schedule` workflow rule and the `SCHEDULED_TASK` selector are deleted from `.gitlab-ci.yml` entirely, so the four schedules cannot be created by accident and the file no longer reads as though they run. Reversible, but now a deliberate edit rather than a settings toggle.

Item 4 (R-hub) — was already decided in this issue's comments; unchanged.

Against this issue's acceptance criteria, with the one correction the maintainer directive requires:

  - "runs the verify command on every push and merge request" — DELIBERATELY NOT MET, and superseded. Per-push checking is local (`tools/verify.sh standard` in the pre-push hook); the hosted pipeline runs only on `CRAN_PREP=1` or `DEPLOY_PAGES=1`. This criterion was written before the minutes ran out.
  - "'glab mr merge' stops reporting 'No pipeline running'" — WILL NOT BE MET, and should not be. It still reports it (seen merging !15) and that is now the correct output: an MR creates no pipeline by design.
  - README badges — no pipeline-status badge is claimed. OK.
  - cran-comments.md describes the CI that genuinely ran — STILL OPEN. It needs to describe two named pipelines plus the local tiers, and must not imply anything runs per push. This is the one criterion left, and it belongs to the release chain rather than to CI work.

#### 2026-09-05 — bartek@turczynski.pl

Closing done. Every scope item is resolved (1, 3, 4, 5) or deliberately superseded (2, by PSLR-ugxanxne, which now enforces the no-schedules policy in the file rather than by convention). The one acceptance criterion still open — cran-comments.md describing the CI that genuinely ran — is release-chain work, not CI work, and is tracked against the submission phases of PSLR-hrpalwzo. Two criteria in this issue's body are recorded above as deliberately NOT met, because the maintainer directive of 2026-09-05 replaced per-push hosted CI with the local gate.
...
Resolved as BUMP, settled by a unanimous three-way vote (this session's analysis, Codex gpt-5.6-sol high, an independent Claude reviewer). The maintainer review this issue was filed to require is the delta enumeration below.

Shipped in 799964f, merged as 231e2a7 (MR !18).

Pin: 9186eeeda85c (2026-06-13) -> 46ae48ceff01 (2026-09-05 14:17 UTC). Note the head moved once mid-session: it was 69d97f7f at 08:18 UTC this morning and 46ae48ce by the time the bump ran, so the delta below is re-derived against what actually shipped, not against the morning figure.

Delta, measured not guessed: 10212 -> 10323 rules, +140 / -29 (icann 6950, private 3373).

Additions worth naming: *.eth.limo, *.eth.link, *.aivencloud.com, *.cursorusercontent.com, claudeusercontent.com and frame.claudeusercontent.com, mygov.scot, the 'web' gTLD, 72 regional *-01.azurewebsites.net entries, the puter.* and codepen.* families, several ddns/dnshome dynamic-DNS suffixes, and Norwegian renames (audnedal.no, hamarøy.no, karlsøy.no, suohkan.no, tjielte.no, ulstein.no).

Removals worth naming: aivencloud.com (narrowed to the wildcard), adaptable.app, deta.app, deta.dev, ac.tj, biz.at, info.at, audnedaln.no, aquila.it, valdaosta.it, trentinosudtirol.it, mayfirst.org, protonet.io, xnbay.com, uk0.bigv.io, vm.bytemark.co.uk.

Behavior change confirmed live against the rebuilt index, not inferred:
  x.ac.tj             suffix tj            (was ac.tj)
  proj.aivencloud.com suffix proj.aivencloud.com, registrable NA
  example.web         suffix web, registrable example.web
  a.foo.audnedal.no   suffix audnedal.no
  site.mygov.scot     suffix mygov.scot

No profile change rides along. The regeneration recorded uts46-nontransitional-std3-v2 / Unicode 17.0.0, which is what the tree already carried (checked by loading the pre-bump R/sysdata.rda out of git and comparing), so the punycoder 1.3.0 question on PSLR-fjkaqckg is untouched by this.

data-raw/update_psl.R's default_commit is bumped alongside the artifacts, so the in-tree pin and the bundled bytes cannot drift apart. This is the first time that constant has moved since the initial bundling.

Verification: tests/testthat/fixtures/psl-vectors.txt is byte-identical between the two upstream commits (the official CC0 vectors did not change), and tools/verify.sh standard passed on the branch — lint clean, 2035 pass / 0 fail / 0 skip. NEWS.md bullet added, flagged as changing query results.

Caveat for the release chain: the pin will be stale again by the time pslr is submitted, since upstream moves daily. Whether to re-bump immediately before submission is a release-checklist question, not a reason to hold this one.

#### 2026-09-05 — bartek@turczynski.pl

Follow-up on this bump, found by the first `tools/verify.sh full` run after it.

`799964f` regenerated the bundled snapshot but not `README.md`, which embeds live
`psl_status()` output. Between that commit and MR !23 the front page advertised
the provenance of the snapshot this bump replaced:

    checksum:     sha256:54fb5c65a1e2...      shipped 00dda6fa8406...
    content date: 2026-06-13 21:47 UTC        shipped 2026-09-05 14:17 UTC
    retrieved:    2026-07-26 13:00 UTC        shipped 2026-09-05 14:40 UTC

Regenerated in commit ba206f4. The new values match what this bump's own commit
message recorded for 46ae48ce, so it is purely the regeneration step that was
omitted — three lines, all snapshot provenance, nothing else in the README moved.

No blame in this: nothing could have caught it at push time. The README drift
check exists only in the `full` tier, the pre-push hook runs `standard`, and the
hosted `readme` job needs `CRAN_PREP=1`. The bump landed after the day's last
recorded `full` run, so the window was structural. The gap itself — a snapshot
bump invalidates the README and no step in `data-raw/update_psl.R` regenerates
it — is filed as **PSLR-qjsszyig**.

Worth noting against D17 ("a bundled-snapshot change is a released version"):
this is the same shape as PSLR-girwpagy, two surfaces disagreeing about the same
artifact where the wrong one looks authoritative.
...
Maintainer directive 2026-09-05: testing is two pipelines — one local, one remote that fires only for CRAN prep. Implemented in 8a0dc2d (MR !15, merged as 3501440).

The `workflow:` rules this issue established admitted five triggers: a `v*` tag, `web`, `api`, `trigger`, and `schedule`. That is broader than "runs when we prep for CRAN" and is how a project on a free minutes quota spends them without deciding to. Now exactly two pipelines, each named by the variable that starts it:

    CRAN_PREP=1     the pre-submission gate
    DEPLOY_PAGES=1  republish the documentation site

A run with neither creates no pipeline at all. The `v*` rule is dropped as a paid duplicate of the CRAN-prep run that precedes it. The `schedule` rule and the `SCHEDULED_TASK` selector are dropped rather than left dormant — this issue's "do NOT create them" is now enforced by the file instead of by remembering.

The real defect found while doing it, and it was not cosmetic: the per-job `rules:` blocks were written for the pre-ugxanxne model and would have outlived it wrongly. `full-check`, `osv-audit` and `security-audit` each fell through to `when: manual`, so the R matrix and both audits would NOT have run in a CRAN-prep pipeline — the gate would have quietly been lint + check + coverage. Each job now extends one of three rule sets (blocking / advisory / manual), and the effective rules were read back from the CI lint API's merged YAML rather than trusting that `extends` composed as intended.

`security-audit` and `psl-upstream-check` are advisory (`allow_failure: true`) because they exit on a missing credential; `codemeta` stays manual because it commits to main. That matches the evidence from the last full remote run (pipeline 2745702662): every job green except `security-audit` and `codemeta`, both purely for absent secrets.

Both remaining boxes on this issue are still open and both are yours:

  - [ ] OSSINDEX_USER / OSSINDEX_TOKEN in ~/.Renviron. `tools/verify.sh standard` confirms the gap is live — test-security.R:18 skipped with "OSS Index credentials not set". The full tier is vacuous until they are set (PSLR-njeqwltb).
  - [ ] one `full` run recorded. `--staleness` reports STALE, last passed 27 days ago.

Docs updated to match: docs/git-workflow.md gains a "The remote pipeline" section stating what each leg can and cannot do (no macOS, no Windows, no sanitizers remotely; no three-R-version matrix locally), and CONTRIBUTING.md no longer instructs the reader to create a `SCHEDULED_TASK=psl-upstream` schedule that must not exist.

#### 2026-09-05 — bartek@turczynski.pl

Checked both remaining boxes today. One is satisfied; the other is NOT, and could
not have been satisfied the way this issue says to satisfy it.

**[x] one `full` run recorded, establishing the baseline stamp** — done.
`.verify-stamp` holds `1788603970` = 2026-09-05 12:26:10, and
`tools/verify.sh --staleness` reports `FRESH  full check last passed 0 days ago`.
The mechanism works exactly as designed.

**[ ] OSSINDEX_USER / OSSINDEX_TOKEN in ~/.Renviron so the full tier is not
vacuous** — NOT satisfied, and the instruction as written cannot satisfy it.
The credentials ARE correctly installed in `~/.Renviron`:

    $ Rscript -e 'cat(nchar(Sys.getenv("OSSINDEX_TOKEN")))'
    61

But `tools/verify.sh` is bash, and `~/.Renviron` is read by R at startup, not by
any shell:

    $ echo "[${OSSINDEX_TOKEN:-UNSET}]"
    [UNSET]

So `run_security()` takes its skip branch on every `full` run and the OSS Index
audit has not executed once — while its own warning tells the user to put the
token in `~/.Renviron`, i.e. instructs the exact action that does not work. The
R side would succeed if it were ever reached; only the bash guard in front of it
is looking in the wrong place.

This is PSLR-njeqwltb re-entering through a different door. That issue was filed
because the audit was vacuously green; the guard added to fix it is correct in
intent and wrong in location, and the net effect is the same — the
dependency-advisory half of what `full` is supposed to have taken over from the
weekly CI schedules has never run.

Worse for the `cran` tier, which makes the same test hard (`fail ...; exit 1`):
a pre-submission run should abort on a correctly configured machine. Worth
establishing how the 2026-09-05 cran run (MR !16) got past it — if the token was
exported by hand in that shell, the tier has only ever passed via an
undocumented manual step.

Filed as **PSLR-lidgnomk** (high) with the fix: resolve the value from
`${R_ENVIRON_USER:-$HOME/.Renviron}` when the shell variable is unset, export it
for the child R process, keep an exported variable as the override, and keep
both tier behaviours (soft in `full`, hard in `cran`) intact so the vacuous pass
does not come back. This issue's second box should be ticked when that lands and
a `full` run actually executes the audit — not before.

Keeping this in-progress rather than closing it: the box is genuinely unmet.

#### 2026-09-05 — bartek@turczynski.pl

Both boxes are now ticked. Closing.

**[x] OSSINDEX_USER / OSSINDEX_TOKEN so the full tier is not vacuous.** The
credentials were correctly installed in `~/.Renviron` all along; the guard in
front of the audit was reading the shell environment, which never sees that
file, so the audit was skipped on every run (PSLR-lidgnomk). Fixed in MR !22:
the guard now resolves from `${R_ENVIRON_USER:-$HOME/.Renviron}` when the
environment does not already define the value, an exported variable still wins
so CI is unaffected, and both tier behaviours are unchanged — soft warn in
`full`, hard fail in `cran`.

**[x] one `full` run recorded, establishing the baseline stamp.** Recorded, and
now recorded on a run that actually exercises every step. `tools/verify.sh full`
end to end, exit 0:

    lint            no lints
    tests           passed
    R CMD check     1 NOTE (the anonymous GitLab BugReports 404, pre-explained
                    in cran-comments.md)
    NEWS/version    agree
    README          matches README.Rmd
    coverage        reported
    OSV audit       no advisories
    OSS Index       no advisories
    upstream PSL    bundled snapshot is current

The OSS Index line reads "no advisories", not "skipped". That is the end-to-end
confirmation this box was waiting for: the dependency-advisory half of what
`full` took over from the weekly CI schedules is now genuinely running rather
than reporting a soft warning every time.

Two things this run surfaced that are their own issues rather than blockers here:

  - **PSLR-qjsszyig** (high) — the first `full` run failed its README drift
    check. `799964f` bumped the bundled snapshot but did not regenerate
    `README.md`, which embeds live `psl_status()` output, so the front page had
    been advertising the previous snapshot's checksum and content date.
    Regenerated in MR !23; the process gap that allowed it is filed separately.
    Worth reading as evidence FOR this issue's design: the `full` tier caught a
    real provenance defect that no per-push gate could have, which is exactly
    what it was moved behind the staleness nudge to do.
  - Still unresolved, and recorded rather than guessed: nothing in the tree
    explains how the 2026-09-05 `cran` run got past the old hard guard, which
    has existed since 8ada982 and sits before every step of that tier. Either
    the token was exported ad hoc in that shell — in which case the audit did
    run that day — or that run was not a literal end-to-end `tools/verify.sh
    cran`. No run log survives to distinguish them.

The accepted trades in the description stand unchanged: the gate is on one
machine, `--no-verify` reaches main unchecked, and `psl-upstream-check` goes
stale because the world moved rather than because the tree changed.
...
## PSLR-uwcwkfwm: R3: run the cran gate for rurl 3.0.0 in repair and remote mode
...
REWRITTEN 2026-08-12: retargeted from 2.7.0 to 3.0.0, and the diagnosis in the
2026-07-19 comments below is now HISTORY, not a live description of the tree.

Invoke the `cran` skill for rurl only, in repair mode and remote mode. Run every
applicable stage: scratch regeneration/load/README comparison, lint/tests,
coverage, `devtools::check(cran = TRUE, remote = TRUE, manual = TRUE)`,
goodpractice, win-builder, and current supported R-hub platforms.

Do not submit. Wait for all asynchronous results. If any repair changes
dependency metadata, generated files, public behavior, or the tarball, rerun R2
and every invalidated stage.

### Read the old comments as a record, not as a plan

The 2026-07-19 R3 run found 13 Windows test failures and attributed them to two
root causes. **One of the two no longer exists.**

- **libcurl-version divergence — GONE.** That comment says "rurl is pure R,
  delegates URL parsing to the curl package's `curl_parse_url()` = libcurl
  (local 8.14.1)". rurl has no curl dependency; the parser is in-tree, and
  `tools/curl-zero-gate.R` proves the absence on every run. Do not go looking
  for that seam.
- **Fixture encoding — verify, do not assume.** The `read.csv()` without
  `fileEncoding = "UTF-8"` finding may or may not still hold; the fixture and
  its reader have both moved since. Re-measure before acting.

Also stale: the maintainer-email-change NOTE
(`bartek+rurl@` -> `bartek@`) and the "RSPM has not synced punycoder 1.2.1"
transient. Both are resolved.

### Preconditions before this runs

`Rscript tools/dependency-resolvability-gate.R` must be green in rurl. It is not
today: RURL-ovgasmea.

Done: `cran` reports a complete go, every remote check is green, and the exact
pushed commit and evidence are recorded.
...
Retargeted 2026-08-12 from rurl 2.7.0 to 3.0.0 (owner ruling: 3.0.0, no further bumps).

The substantive edit is not the version. The 2026-07-19 R3 comments on this issue are the most detailed technical record in the pslr tracker, and HALF OF IT IS NOW FALSE. It states that rurl delegates parsing to curl_parse_url() and diagnoses 13 Windows failures as libcurl-version divergence across three call sites. curl is no longer a dependency of rurl in any field, the parser is in-tree, and tools/curl-zero-gate.R fails the build if a single reference reappears.

That mattered enough to say explicitly in the body: an agent handed this issue would have spent the run looking for a seam that has been deleted, and would have read a real, careful, measured comment as current. The comments are kept -- they are the record of why the parser was replaced -- but the body now frames them as history.

The second root cause (read.csv without fileEncoding='UTF-8') is NOT asserted either way here. It was true in July; the fixture and its reader have both moved since, and I did not re-measure it. Flagged as verify-before-acting rather than carried forward as a finding, because a stale finding stated confidently is what this sweep is for.
...
Step 1 DONE, merged as 12e3169 (MR !14). Two corrections to this issue's body, both measured 2026-08-12:

1. The live entry was `gitlab::bart-turczynski/punycoder`, NOT `bart-turczynski/punycoder` as written above. The host prefix was already correct -- **only the ref was missing**. The 'pick one convention, rurl pins and pslr floats' framing in step 1 is also overtaken: rurl has no `Remotes:` field at all any more (it was dropped in 0df6cce once both siblings reached CRAN above their floors), so there is no opposing convention left to reconcile.

2. Pinned to punycoder main `ec2643c7783e3aed3067ea85238788501c51b282`, not tag `v1.2.1`. v1.2.1 predates Unicode 17 and fails four pslr tests against current sysdata.

The pin is the full 40-character sha deliberately. An abbreviated sha is only checkable by matching it against the refs a remote advertises, so it stops being verifiable the moment it is no longer some ref's tip; a full object id stays fetchable. This was not a guess -- the naive check in rurl's dependency-resolvability gate could not resolve a sha pin at all and reported this very change as unserved; that defect is RURL-vdbfgkad, now fixed, and the gate reports pslr PASS.

STILL OPEN here: step 2 (element-wise diff of pslr's corpora against punycoder main vs v1.2.1 -- the check floating has been skipping), step 3 (audit for profile-token use as a cache or reproducibility key), and step 4 (raise Imports to punycoder (>= 1.3.0) once punycoder publishes it).
...
## PSLR-wcjcywea: CI installs pandoc from a hardcoded amd64 .deb: every job dies on an arm64 runner

**Status:** done

### Description

`.gitlab-ci.yml` installs pandoc by downloading a `.deb` whose filename hardcodes
the architecture:
...
Three copies, at lines 204-206 (`.r-image`), 231-233 and 255-257. The repetition
is deliberate and documented -- the pre-commit `check-yaml` hook parses this file
with a standard YAML loader, which rejects GitLab's `!reference` tag -- so a fix
has to touch all three.

### Why it is latent rather than visible

GitLab.com's shared runners are x86_64, so the literal is correct there and the
bug never fires. It fires on any arm64 runner. That is not hypothetical: the
namespace ran out of compute minutes on 2026-08-07 (PSLR-ugxanxne), and the
documented fallback is a local Docker `gitlab-runner` on an Apple Silicon
machine. Docker there resolves `rocker/r-ver:${R_VERSION}` to arm64,
gitlab-runner's Docker executor cannot request a platform, and `dpkg -i` on an
amd64 `.deb` then fails -- taking out every job that inherits the template,
which is every job that needs pandoc.

The current workaround is manual and has to be repeated for each R version used:
read the amd64 digest with `docker manifest inspect`, `docker pull` by digest,
then `docker tag` it back to the plain tag, for `4.6` plus `4.5` and `devel` for
the `full-check` matrix. That is per-run toil standing in for a one-line fix.

### Fix

Derive the architecture instead of asserting it. `dpkg --print-architecture`
reports `amd64` or `arm64` in the exact spelling pandoc's release assets use, and
pandoc publishes both, so the download URL can be built from it. The result is
correct on GitLab's fleet and on a local arm64 runner without either being a
special case.

### Done when

- No `-amd64.deb` literal remains in `.gitlab-ci.yml`.
- The architecture is derived at run time in all three `before_script` copies.
- The change is a pure substitution: nothing else about the pandoc pin moves.
  `PANDOC_VERSION` stays pinned for the reason the file already documents (the
  `readme` job compares generated output, and distro pandoc renders pipe tables
  differently).

### Provenance

Surfaced from the Claude auto-memory store during PSLR-fqdepqem, which recorded
it as "a latent portability bug in the committed CI config, invisible on
GitLab's x86 fleet ... not yet filed". Re-verified against the tree before
filing: the three literals are present at the lines named above.
...
Landed in MR !19, commit 0522d16, merged to main.

All three `before_script` copies now build the URL as
`pandoc-${PANDOC_VERSION}-1-$(dpkg --print-architecture).deb`. Command
substitution rather than a variable, because GitLab's runner-side expander only
recognises `$NAME`/`${NAME}` and leaves `$( )` for the shell to evaluate inside
the container it has to describe — an idiom this file already relies on in the
`news-version` and `psl-upstream-check` jobs, so it is precedent rather than a
new bet.

Verified independently rather than accepted from the report: pandoc 3.10 (the
pinned version) publishes both `pandoc-3.10-1-amd64.deb` and
`pandoc-3.10-1-arm64.deb`, both HTTP 200; the folded scalar was reconstructed
and evaluated under a stubbed `dpkg` returning each architecture in turn and
produced a live URL each time; `yaml.safe_load` parses the file and goes red on
a deliberate de-indent, which is the loader the `check-yaml` pre-commit hook
uses; no `-amd64.deb` literal survives.

Pure substitution otherwise — `PANDOC_VERSION` stays pinned at 3.10, no apt
switch, no version bump, and the three copies stay copies rather than becoming a
`!reference` the standard YAML loader would reject.

Gate: `tools/verify.sh standard` — lint clean, 2035 tests, 0 failures, identical
to the `main` baseline. No NEWS bullet; a CI-config fix is not user-facing.

Residual, and it cannot be closed from here: `dpkg --print-architecture`
returning `arm64` on an arm64 runner is documented Debian behaviour but was not
executed. The first real run on the local Apple Silicon runner confirms it — and
that run is the one that no longer needs the manual per-tag digest pinning this
issue was filed to retire.




## PSLR-wclmdkju: Publish the pkgdown site from a local runner and keep agent-instruction files out of it
...
Agent-instruction files excluded from the site: merged as 4b9f5b4 (MR !13). pkgdown:::package_mds() hardcodes its skip list, so _pkgdown.yml cannot exclude a file — the pages job now moves AGENTS.md, CLAUDE.md, FP_AGENTS.md and FP_CLAUDE.md aside between local_install and build_site. Clean local build verified: no agent pages, search index 217 -> 202 entries.

Deploy unblocked without spending money: registered project runner 54981995 (pslr-local-docker, docker executor) on the local machine. Pipeline 2745695892 is running the pages job on it — confirms a project runner is NOT subject to the namespace compute-minutes block that failed all 13 jobs in 2745523194.

#### 2026-08-09 — bartek@turczynski.pl

Site is live at https://bart-turczynski.gitlab.io/pslr/ (HTTP 200). pages job succeeded in 218s on the local runner; Pages deployment recorded 2026-08-09T21:35:40Z.

Verified in production: AGENTS.html, CLAUDE.html, FP_AGENTS.html and FP_CLAUDE.html all 404; reference/, news/, articles/ and search.json all 200; live search index is 202 entries with 0 referencing agent files.

The URL DESCRIPTION declares as homepage now resolves, which clears the pre-CRAN blocker noted in the previous session.

#### 2026-08-09 — bartek@turczynski.pl

Full 13-job pipeline 2745702662 run on the local runner (user-triggered). Result: 10 success, 2 failed, 1 manual.

Green: lint, news-version, check (389s, the strict --as-cran gate), readme, coverage (95.27%), full-check devel/4.6/4.5, osv-audit, pages (183s).

Failed, both on missing CI/CD variables — the project has zero variables set, nothing migrated from GitHub:
- security-audit: OSSINDEX_USER / OSSINDEX_TOKEN absent. Failing loudly is correct behaviour per PSLR-njeqwltb, which fixed the opposite bug (vacuously green).
- codemeta: PSL_BOT_TOKEN absent, needed to push a regenerated codemeta.json to main.

Both only ran because the commit touched .gitlab-ci.yml; their rules are otherwise manual + allow_failure. A routine docs redeploy would have been green without answering these questions.

Also required pinning rocker/r-ver:4.5 and :devel to their amd64 digests locally — .r-image hardcodes pandoc-3.10-1-amd64.deb, so an arm64 runner fails at dpkg -i. That is a latent portability bug in .gitlab-ci.yml, invisible on GitLab's x86 fleet.
...
Resolved as **D21** in docs/decisions.md (MR !21, commits 2f6cd71 + 828b1f4).
No code changed under this id; `DESCRIPTION` is untouched and still declares
`R (>= 4.1.0)`.

Ruling: keep `\(x)` as house style and keep the 4.1.0 floor as a deliberate,
accepted cost — option (a). Settled by a four-voter panel (2x Codex gpt-5.6-sol
high, 2x Claude Opus high), 3-1.

This issue's central premise was false when it was filed, which is the part
worth recording here. It says "Nothing mandates `\()` ... no formatter or linter
mandates `\()`". But `git log -S` dates the sentence "`\(x) ...` for one-line
anonymous functions; `function(x) { ... }` otherwise" to 4998756 (2026-06-28),
which wrote it into AGENTS.md three weeks BEFORE this issue was filed on
2026-07-19; baf02e2 (2026-08-09) only relocated it into docs/r-conventions.md.
The rule was house style the entire time. The same section also mandates the
base pipe `|>`, which is likewise R 4.1.0+, so a 4.0.0 floor is fiction the
moment anyone follows the conventions document — independently of the lambda.

The occurrence count is stale in the other direction: "7 occurrences: R/query.R,
R/matcher.R, R/parser.R" is still exactly right for those three files, but the
tree now holds 54 sites across 15 files in R/ and 61 across 15 in tests/ — 115
in 30 files. A reversion confined to R/ would have advertised a 4.0.0 floor that
the suite proving the package correct had never been run against.

The dissent is recorded in D21 rather than summarized away. One voter ruled for
reverting: Debian 11 / ELTS users pinned to R 4.0.4 are a small but real
beneficiary population, and rurl declaring 4.0.0 while transitively requiring
4.1.0 through pslr is substantively misleading rather than cosmetic. D21 grants
the second point and explains why it is rurl's to fix by declaring its own
effective floor, not pslr's to fix by lowering one it honestly needs. Worth
carrying to the rurl tracker on its own terms.

This issue's instruction to decide "at the NEXT library modification (not a
standalone churn commit)" is superseded and D21 says so: it was written for a
7-site change, does not scale to 115, and ruling (a) needs zero code edits, so
there is nothing left to piggyback on.

Reopening condition, per D21: a named user on a supported platform inside
`[4.0.0, 4.1.0)` — and any reopening must budget for `|>` as well, since both
rules imply the same floor.
...
## PSLR-wydqlubg: Should README.md print the snapshot checksum and timestamps at all? (panel split on PSLR-qjsszyig)

**Status:** todo | **Parent:** qjsszyignorsrtcdkztmhkrjreuaxfwo

### Description

Raised by the panel that ruled on PSLR-qjsszyig. Two of four voters argued the
real defect there is not the missing regeneration step at all, but that
`README.md` embeds volatile provenance no other generated file does. That
question survives PSLR-qjsszyig's fix and is worth deciding on its own.

### The claim

`README.Rmd`'s `freshness` chunk evaluates `psl_status()`, whose output includes:

    checksum:     sha256:00dda6fa8406...
    content date: 2026-09-05 14:17 UTC
    retrieved:    2026-09-05 14:40 UTC

The argument for removing them: a long-lived landing-page example is coupled to
quarterly-changing bundled data, so every snapshot bump churns the front page,
and anyone who wants exact provenance can call `psl_status()` or `psl_version()`
themselves. The proposal was to keep the stable half -- the status state, its
explanation, the snapshot kind and the source URL -- and drop or placeholder the
three volatile fields.

### The counter, which is why this was not adopted under PSLR-qjsszyig

Both opposing voters found, and it checks out in the tree, that `README.Rmd`
already carries a `freshness-setup` chunk whose stated purpose is exactly this
concern:

    # Render against an empty snapshot cache so this block is byte-identical on
    # every machine (the CI gate re-renders README.md and diffs it). Without
    # this, the output would reflect whatever the rendering machine happens to
    # have refreshed.

So the values are not render-volatile -- the author already neutralised the
machine-to-machine half. The only input that moves them is a snapshot
regeneration, which D17 records as a released version. On that reading the
checksum is load-bearing evidence for a provenance claim the README makes about
itself, and replacing it with a placeholder would make the front page less
honest, not more stable.

### The part both sides agreed on

`retrieved:` is the wall-clock second the maintainer happened to run the
pipeline. It has no reader value, it is the field most likely to churn for no
reason, and both opposing voters named it unprompted as the one candidate worth
trimming -- while saying it is a `psl_status()` design question rather than a fix
for the sync bug.

That is the narrow, uncontested version of this issue, and probably where it
should land.

### The decision

Three positions, in increasing scope:

1. **Leave it.** The provenance block is evidence, the sync problem is fixed by
   PSLR-qjsszyig, and there is nothing left to do.
2. **Trim `retrieved:` only** -- from `psl_status()`'s printed output, or from
   the README rendering of it. Decide which: changing the print method is a
   user-visible API change and needs a NEWS bullet; changing only the README is
   a doc change but re-introduces a divergence between what the README shows and
   what a user sees.
3. **Drop all three volatile fields from the README block**, as the two voters
   proposed.

Position 2 is the one with an argument nobody contested. Note that it interacts
with PSLR-qjsszyig: the further this goes, the less that fix has to do, though
it stays correct either way.

### Not urgent

Nothing is broken. PSLR-qjsszyig closes the defect that prompted this. This is a
design question about what the front page should assert, filed so the panel's
...
## PSLR-wzrevacl: R5: verify rurl 3.0.0 is accepted and live on CRAN

**Status:** in-progress | **Parent:** kwareossnnzmzlybtygrronedzqhqyss

### Description

Verify rurl 3.0.0 in the official CRAN master index, package page, published
`DESCRIPTION`, source tarball, and CRAN check results. Install it into an empty
CRAN-only library and confirm the live graph resolves to acceptable pslr and
punycoder versions with no `Remotes` and no alternate repository.

REWRITTEN 2026-08-12: retargeted from 2.8.0/2.7.0 to 3.0.0; title and body had
disagreed since 2026-07-26.

Create the immutable release tag according to repository policy; compare the
CRAN tarball with the tagged source and explain only CRAN-added metadata
differences.

**"and GitHub release" is struck.** The forge is GitLab (RURL-sgkplmot) and the
GitHub account is suspended. Tag on GitLab.

**Tagging is not optional here and is easy to skip.** rurl's tags are sparse --
2.0.0 and 2.1.0 were shipped and never tagged. If 3.0.0 goes to CRAN untagged,
the released source has no immutable in-repo counterpart, and the
dependency-resolvability gate loses the tag it resolves sibling floors against.

Done: rurl 3.0.0 is live and cleanly installable from CRAN, provenance is
complete, and `v3.0.0` exists as an annotated tag.

### Comments

#### 2026-08-12 — bartek@turczynski.pl

Retargeted 2026-08-12 to 3.0.0 and corrected two forge-shaped errors the version fix alone would have left in place.

1. 'Create the immutable release tag AND GITHUB RELEASE' -- the forge moved to GitLab (RURL-sgkplmot) and the GitHub account is suspended. Struck.

2. Added the tagging hazard explicitly, because it is not hypothetical in this repo: rurl shipped 2.0.0 and 2.1.0 with no git tag of either. An untagged 3.0.0 would also break the new dependency-resolvability gate, which resolves sibling floors against CRAN releases and git tags -- pagerankr's 'rurl (>= 3.0.0)' is UNSATISFIABLE today precisely because no such tag or release exists.

#### 2026-09-08 — bartek@turczynski.pl

Live frontier as of 2026-09-08 21:00 UTC. rurl 3.0.1 (SHA 0ca8f096) is queued at CRAN -- submitted and email-confirmed, see PSLR-lararika.

Nothing to do but wait for CRAN. What to watch, in order:

1. AUTO-CHECK, typically under an hour. CRAN emails the incoming pretest result. A pass moves the tarball to the human queue; a fail returns the same kind of platform-specific report that 3.0.0 got.
2. HUMAN REVIEW, typically 1-10 working days for a package with a NOTE. The only expected NOTE is CRAN incoming feasibility -- new maintainer address plus the BugReports check -- and cran-comments.md addresses each component.
3. ACCEPTANCE. Verify live rather than assuming: the package page at https://CRAN.R-project.org/package=rurl showing 3.0.1, and available.packages() resolving it.

This issue closes on step 3 with that verification pasted in, not on the acceptance email alone.

Note the title says 3.0.0; the submitted version is 3.0.1. See PSLR-lararika for why.
...
## PSLR-xogfazqo: Update the live OpenSSF badge entry on bestpractices.dev to match the corrected local file

**Status:** todo | **Parent:** cftgnuxohcnqkjplqcuiqcnnctsjqbix

### Description

The local `.bestpractices.json` was corrected under PSLR-cftgnuxo (MR !20). This
issue is the half a badge reviewer actually reads: the live entry at
https://www.bestpractices.dev/projects/13430, which still carries the old text
and can only be edited by hand on the web.

### What has to change there

The same six cadence justifications, plus one status flip. Take the wording from
`.bestpractices.json` on main rather than re-deriving it — the file and the live
entry should say the same thing, and the file is now the correct one.

  - `contribution_requirements` — the gate is `tools/verify.sh` local pre-push, not CI.
  - `test_most` — the `coverage` job runs under `CRAN_PREP=1`, not every hosted pipeline.
  - `warnings` — `R CMD check --as-cran` is not in the `standard` tier.
  - `static_analysis_often` — stays Met, but the justification must state that the
    hook is local, unverifiable by a reviewer, and bypassed by `--no-verify`.
  - `dynamic_analysis_enable_assertions` — the sanitize tier is local Docker, not a hosted job.
  - `test_continuous_integration` — **flip the status to Unmet.** This is the one
    substantive change and the reason this issue is not cosmetic: the live entry
    currently claims per-push, tag-triggered and scheduled CI, none of which
    exist. Leaving a false Met is worse than an honest Unmet.

Three further live claims are false and were corrected in the file: the invented
`LINTR_ERROR_ON_LINT=true` under `warnings_strict`, goodpractice under
`static_analysis`, and `R CMD check` under sanitizers in `dynamic_analysis`.

### Why the badge survives

Verified on 2026-08-09 against the passing-level criteria: `dynamic_analysis`,
`dynamic_analysis_unsafe`, `dynamic_analysis_enable_assertions`,
`static_analysis_often` and `test_continuous_integration` are all SUGGESTED, not
MUST. The project is `visibility: public` with `pages_access_level: public`, so
the six MUST criteria a private repository would fail are satisfied. Marking
`test_continuous_integration` Unmet does not drop the badge, and the README badge
at line 22 stays.

### Done when

The live entry's six justifications match `.bestpractices.json` on main, and
`test_continuous_integration` reads Unmet there.

### Note

Only the maintainer can do this — it needs a logged-in session on
bestpractices.dev. No code or repository change is involved.
...
## PSLR-zibafvbq: cran tier reports a pass on an R CMD check that aborted

**Status:** done

### Description

Found by a full `tools/verify.sh cran` run on 2026-09-05.

### What happened

The local as-cran check died on its first step and the tier called it clean:

    * checking CRAN incoming feasibility ...Warning in readRDS(con) :
      URL 'https://CRAN.R-project.org/src/contrib/Meta/archive.rds': Timeout of 60 seconds was reached
...
Zero checks ran past the first. The tier went on to print "cran tier passed — safe to submit".

### Why the existing guard did not fire

run_check() (tools/verify.sh:154) was written for exactly this: it reads 00check.log and fails on "Execution halted" or a missing "Status:" line. Its own comment says "a check that aborted early leaves a log with no findings, which reads as a clean pass".

The cran tier does not call run_check(). It inlines its own rcmdcheck() call (tools/verify.sh:495-499) to set _R_CHECK_CRAN_INCOMING_REMOTE_=true, and the guard did not come with it. So the guard protects `full` and is absent from the one tier that gates a real CRAN submission — and the enabling condition, a slow CRAN fetch, is one that _R_CHECK_CRAN_INCOMING_REMOTE_=true makes *more* likely, not less.

### Fix

Give run_check() a `remote` argument and route both tiers through it, so there is one definition of "the check ran to completion". Keep the two real differences: the remote form passes args = "--as-cran" (builds the manual) where the default passes c("--no-manual", "--as-cran").

Also raise the child process's download timeout: R's default `timeout` option is 60s and rcmdcheck runs the check in a separate R process, so the parent's options() do not reach it. R_DEFAULT_INTERNET_TIMEOUT does.

### Acceptance criteria

- The cran tier fails, loudly, when R CMD check aborts before reaching a Status line.
- No rcmdcheck call in tools/verify.sh bypasses the 00check.log guard.
- A cran-tier run completes the CRAN incoming feasibility check rather than timing out on it.
...
Fixed in MR !16. The 00check.log guard is a shared string and both tiers go through run_check, so no rcmdcheck call bypasses it; the remote form sets R_DEFAULT_INTERNET_TIMEOUT=300. Verified: the shipped guard rejects an 'Execution halted' log and a log with no Status line and passes a complete one, and a real run_check remote reached Status: 1 NOTE with incoming feasibility completing in 28s.

#### 2026-09-05 — bartek@turczynski.pl

Cross-tracker link: the same defect exists in the /cran agent skill and is filed there as SKILL-dhgxrnap (agent-skills, skills/cran/shared/SKILL.md:135 and :158 — devtools::check(..., error_on = "never") gated on '0 errors and 0 warnings', with no 00check.log inspection anywhere in that repository). The skill instance is more severe: its entire output is a go/no-go, so the failure mode emits a false GO, and it applies to every package it is pointed at. Reference fix for it is this issue's commit a9e7918. No code in pslr depends on that issue.
