...
# reports nothing" (SEOR-fkvlzltx, deciding SEOR-feqgnbwt route (a)). For pslr
# the two contracts coincide *today*: the hard dependency closure is 6
# packages (measured 2026-09-10, see test-security.R), and a live audit on
# 2026-09-24 reported zero advisories, so there is nothing to dispose of and
# the list is empty. `curl`, which carries the two advisories that sitemapr
# allow-lists, is absent from pslr's hard dependency tree entirely.
#
# No row is added ahead of need (SEOR-sxvcbuia): rule B below fails a row that
# is not currently reported, so an anticipatory row is stale on arrival and
# would fail the gate on the day it is added. The machinery is what is
# portable; the rows are not. Adding one later is a one-block edit to
# `oss_index_allowlist`.
#
